...
mod test_x_result {
    ...
    fn test_assert_program_args_stderr_lt_as_result_x_success() {
        ...
        let b_args: [&str; 0] = [];
        let x = assert_program_args_stderr_lt_as_result!(&a_program, &a_args, &b_program, &b_args);
        assert_eq!(x.unwrap(), ());
    }
...
macro_rules! assert_program_args_stderr_lt {
    ($a_program:expr, $a_args:expr, $b_program:expr, $b_args:expr $(,)?) => ({
        match assert_program_args_stderr_lt_as_result!($a_program, $a_args, $b_program, $b_args) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a command (built with program and args) stderr string is equal to another.
///
/// This macro provides the same statements as [`assert_program_args_stderr_lt`],
