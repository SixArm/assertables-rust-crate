...
macro_rules! assert_fn_ok_gt_as_result {
    ($a_function:path, $a_input:expr, $b_function:path, $b_input:expr $(,)?) => ({
        ...
        if !a_is_ok || !b_is_ok {
            Err(format!(
                concat!(
                    "assertion failed: `assert_fn_err_gt!(left_function, left_input, right_function, right_input)`\n",
                    "  left_function label: `{}`,\n",
                    "     left_input label: `{}`,\n",
                    "     left_input debug: `{:?}`,\n",
            ...
            if a_ok > b_ok {
                ...
                Err(format!(
                    concat!(
                        "assertion failed: `assert_fn_ok_gt!(left_function, left_input, right_function, right_input)`\n",
                        "  left_function label: `{}`,\n",
                        "     left_input label: `{}`,\n",
                        "     left_input debug: `{:?}`,\n",
                        " right_function label: `{}`,\n",
...
mod test_x_result {
    ...
    fn test_assert_fn_ok_gt_as_result_x_success_because_gt() {
        let a: i32 = 2;
        let b: i32 = 1;
        let x = assert_fn_ok_gt_as_result!(example_digit_to_string, a, example_digit_to_string, b);
        assert!(x.is_ok());
        assert_eq!(x, Ok(()));
    }
...
macro_rules! assert_fn_ok_gt {
    ($a_function:path, $a_input:expr, $b_expr:expr $(,)?) => ({
        match assert_fn_ok_gt_as_result!($a_function, $a_input, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
