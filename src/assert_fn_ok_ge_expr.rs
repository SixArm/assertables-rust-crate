...
macro_rules! assert_fn_ok_ge_expr_as_result {
    ($function:path, $a_input:expr, $b_expr:expr $(,)?) => ({
        ...
        if !a_is_ok {
            Err(format!(
                concat!(
                    "assertion failed: `assert_fn_ok_ge_expr!(left_function, left_input, right_expr)`\n",
                    " left_function label: `{}`,\n",
                    "    left_input label: `{}`,\n",
                    "    left_input debug: `{:?}`,\n",
                    "    right_expr label: `{}`,\n",
...
mod test_x_result {
    ...
    #[test]
    fn test_assert_fn_ok_ge_expr_as_result_x_success_because_gt_expr() {
        let a: i32 = 2;
        let b = String::from("1");
        let x = assert_fn_ok_ge_expr_as_result!(example_digit_to_string, a, b);
        assert!(x.is_ok());
        assert_eq!(x, Ok(()));
    }

    #[test]
    fn test_assert_fn_ok_ge_expr_as_result_x_success_because_eq_expr() {
        let a: i32 = 1;
        let b = String::from("1");
        let x = assert_fn_ok_ge_expr_as_result!(example_digit_to_string, a, b);
        assert!(x.is_ok());
    ...
    #[test]
    fn test_assert_fn_ok_ge_expr_as_result_x_failure_because_lt_expr() {
        let a: i32 = 1;
        let b = String::from("2");
        let x = assert_fn_ok_ge_expr_as_result!(example_digit_to_string, a, b);
        assert!(x.is_err());
...
/// let a: i32 = 2;
/// let b = String::from("1");
/// assert_fn_ok_ge_expr!(example_digit_to_string, a, b);
/// //-> ()
///
...
macro_rules! assert_fn_ok_ge_expr {
    ($function:path, $a_input:expr, $b_expr:expr $(,)?) => ({
        match assert_fn_ok_ge_expr_as_result!($function, $a_input, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
