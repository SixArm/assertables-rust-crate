        ...
        match (&$a, &$b) {
            (a_val, b_val) => {
                ...
                if a_set.is_subset(&b_set) {
                    ...
                    Err(msg_with_left_and_right!("assertion failed", "assert_set_subset!", &a_set, &b_set))
                }
            }
        }
    });
...
mod test_x_result {
    ...
    #[test]
    fn test_assert_set_subset_as_result_x_arity_2_success() {
        let a = [1, 2];
        let b = [1, 2, 3];
        let x = assert_set_subset_as_result!(&a, &b);
        assert_eq!(
    ...
    #[test]
    fn test_assert_set_subset_as_result_x_arity_2_failure() {
        let a = [1, 2, 3];
        let b = [1, 2];
        let x = assert_set_subset_as_result!(&a, &b);
        assert_eq!(
