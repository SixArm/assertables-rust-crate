...
///
/// # fn main() {
/// assert_f_err_string_ne!(digit_string, 10, 20);
/// //-> ()
///
/// # let result = panic::catch_unwind(|| {
/// assert_f_err_string_ne!(digit_string, 10, 10);
/// //-> panic!("…")
/// // assertion failed: `assert_f_err_string_ne!(fn, left, right)`
/// //    left input: `10`,
/// //   right input: `10`,
...
/// # let expect = "assertion failed: `assert_f_err_string_ne!(fn, left, right)`\n   left input: `10`,\n  right input: `10`,\n  left output: `\"10 is out of range\"`,\n right output: `\"10 is out of range\"`";
...
macro_rules! assert_f_err_string_ne {
    ($function:path, $left:expr, $right:expr $(,)?) => ({
        ...
        if !left.is_err() || !right.is_err() {
            panic!("assertion failed: `assert_f_err_string_ne!(fn, left, right)`\n   left input: `{:?}`,\n  right input: `{:?}`\n  left output is_err(): `{:?}`,\n right output is_err(): `{:?}`", $left, $right, left.is_err(), right.is_err());
        } else {
...
mod tests {
    ...
    fn test_assert_f_err_string_ne_x_arity_2_success() {
        let a = 10;
        let b = 20;
        let x = assert_f_err_string_ne!(digit_string, a, b);
        assert_eq!(x, ());
    }

    #[test]
    #[should_panic (expected = "assertion failed: `assert_f_err_string_ne!(fn, left, right)`\n   left input: `10`,\n  right input: `10`,\n  left output: `\"10 is out of range\"`,\n right output: `\"10 is out of range\"`")]
    fn test_assert_f_err_string_ne_x_arity_2_failure() {
        let a = 10;
        let b = 10;
        let _x = assert_f_err_string_ne!(digit_string, a, b);
    }
    ...
    fn test_assert_f_err_string_ne_x_arity_3_success() {
        let a = 10;
        let b = 20;
        let x = assert_f_err_string_ne!(digit_string, a, b, "message");
        assert_eq!(x, ());
    }
    ...
    fn test_assert_f_err_string_ne_x_arity_3_failure() {
        let a = 10;
        let b = 10;
        let _x = assert_f_err_string_ne!(digit_string, a, b, "message");
    }
