...
/// Assert a command (built with program and args) stdout string is equal to another.
///
/// This macro provides the same statements as [`assert_program_args_stdout_eq_other`],
...
///
#[macro_export]
macro_rules! debug_assert_program_args_stdout_eq_other {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_program_args_stdout_eq_other!($($arg)*);
        }
    };
}
