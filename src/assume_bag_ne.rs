        ...
        match (&$left, &$right) {
            (left_val, right_val) => {
                ...
                if left_bag != right_bag {
                    ...
                    Err(format!("assumption failed: `assume_bag_ne(left, right)`\n  left: `{:?}`\n right: `{:?}`", $left, $right))
                }
            }
        }
    });
...
mod tests {
    ...
    #[test]
    fn test_assume_bag_ne_x_arity_2_success() {
        let a = [1, 1];
        let b = [1, 1, 1];
        let x = assume_bag_ne!(&a, &b);
        assert_eq!(
    ...
    #[test]
    fn test_assume_bag_ne_x_arity_2_failure() {
        let a = [1, 1];
        let b = [1, 1];
        let x = assume_bag_ne!(&a, &b);
        assert_eq!(
            x.unwrap_err(),
            "assumption failed: `assume_bag_ne(left, right)`\n  left: `[1, 1]`\n right: `[1, 1]`"
        );
    }

    #[test]
    fn test_assume_bag_ne_x_arity_3_success() {
        let a = [1, 1];
        let b = [1, 1, 1];
        let x = assume_bag_ne!(&a, &b, "message");
        assert_eq!(
            x.unwrap(),
            true
        )
    ...
    #[test]
    fn test_assume_bag_ne_x_arity_3_failure() {
        let a = [1, 1];
        let b = [1, 1];
        let x = assume_bag_ne!(&a, &b, "message");
        assert_eq!(
