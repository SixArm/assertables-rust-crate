//! Assert a delta difference is greater than or equal to a value.
//!
//! Pseudocode:<br>
//! Δ ≥ x
//!
...
///
#[macro_export]
macro_rules! assert_delta_ge_x_as_result {
                    ...
                    Ok(delta) => {
                        if delta >= *x {
                            ...
                            Err(
                                format!(
                                    concat!(
                                        "assertion failed: `assert_delta_ge_x!(a, b, x)`\n",
                                        "https://docs.rs/assertables/9.5.0/assertables/macro.assert_delta_ge_x.html\n",
                                        " a label: `{}`,\n",
                                        " a debug: `{:?}`,\n",
                                        " b label: `{}`,\n",
                                        " b debug: `{:?}`,\n",
...
#[cfg(test)]
mod test_assert_delta_ge_x_as_result {
    ...
    fn gt() {
        ...
        let b: i8 = 13;
        let x: i8 = 2;
        let actual = assert_delta_ge_x_as_result!(a, b, x);
...
///
#[macro_export]
macro_rules! assert_delta_ge_x {
    ($a:expr, $b:expr, $x:expr $(,)?) => {{
        match $crate::assert_delta_ge_x_as_result!($a, $b, $x) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    }};
...
mod test_assert_delta_ge_x {
    ...
    fn ge() {
        ...
        let b: i8 = 13;
        let x: i8 = 2;
        let actual = assert_delta_ge_x!(a, b, x);
    ...
    fn lt() {
        ...
        let x: i8 = 4;
        let result = panic::catch_unwind(|| {
            let _actual = assert_delta_ge_x!(a, b, x);
        });
        let message = concat!(
            "assertion failed: `assert_delta_ge_x!(a, b, x)`\n",
            "https://docs.rs/assertables/9.5.0/assertables/macro.assert_delta_ge_x.html\n",
...
/// Δ ≥ x
///
/// This macro provides the same statements as [`assert_delta_ge_x`](macro.assert_delta_ge_x.html),
...
///
#[macro_export]
macro_rules! debug_assert_delta_ge_x {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_delta_ge_x!($($arg)*);
        }
    };
}
