...
/// # Related
///
/// * [`read_to_string_ne_expr`]
/// * [`read_to_string_ne_expr_as_result`]
/// * [`debug_read_to_string_ne_expr`]
///
#[macro_export]
...
mod tests {
    ...
    fn test_read_to_string_ne_expr_as_result_x_success() {
        ...
        let x = assert_fs_read_to_string_ne_expr_as_result!(&path, &value);
        assert_eq!(x, Ok(()));
    }
...
macro_rules! assert_fs_read_to_string_ne_expr {
    ($a_path:expr, $b_expr:expr $(,)?) => ({
        match read_to_string_ne_expr_as_result!($a_path, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a std::fs::read_to_string() is not equal to an expression.
///
/// This macro provides the same statements as [`read_to_string_ne_expr`],
