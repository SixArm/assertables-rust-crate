...
/// * [`assert_fn_err_ne_expr_as_result`]
/// * [`debug_assert_fn_err_ne_expr`]
///
#[macro_export]
macro_rules! assert_fn_err_ne_expr_as_result {
    ($a_function:path, $a_input:expr, $b_expr:expr $(,)?) => ({
        ...
        if !a_is_err {
            Err(format!(
                concat!(
                    "assertion failed: `assert_fn_err_ne_expr!(left_function, left_input, right_expr)`\n",
                    " left_function label: `{}`,\n",
                    "    left_input label: `{}`,\n",
                    "    left_input debug: `{:?}`,\n",
                    "    right_expr label: `{}`,\n",
...
mod test_x_result {
    ...
    #[test]
    fn test_assert_fn_err_ne_expr_as_result_x_success() {
        let a: i32 = 10;
        let b = String::from("20 is out of range");
        let x = assert_fn_err_ne_expr_as_result!(example_digit_to_string, a, b);
        assert!(x.is_ok());
        assert_eq!(x, Ok(()));
    }

    #[test]
    fn test_assert_fn_err_ne_expr_as_result_x_failure() {
        let a: i32 = 10;
...
/// let a = 10;
/// let b = String::from("20 is out of range");
/// assert_fn_err_ne_expr!(example_digit_to_string, a, b);
/// //-> ()
///
/// let a = 10;
/// let b = String::from("10 is out of range");
...
/// // Panic with error message
/// let result = panic::catch_unwind(|| {
/// assert_fn_err_ne_expr!(example_digit_to_string, a, b, "message");
/// //-> panic!
/// });
/// assert!(result.is_err());
/// let actual = result.unwrap_err().downcast::<String>().unwrap().to_string();
...
macro_rules! assert_fn_err_ne_expr {
    ($a_function:path, $a_input:expr, $b_expr:expr $(,)?) => ({
        match assert_fn_err_ne_expr_as_result!($a_function, $a_input, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a function err() is not equal to an expression.
///
/// This macro provides the same statements as [`assert_fn_err_ne_expr`],
