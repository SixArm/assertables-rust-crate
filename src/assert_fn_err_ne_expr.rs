...
/// # Module macros
///
/// * [`assert_fn_err_ne_expr`](macro.assert_fn_err_ne_expr.html)
/// * [`assert_fn_err_ne_expr_as_result`](macro.assert_fn_err_ne_expr_as_result.html)
/// * [`debug_assert_fn_err_ne_expr`](macro.debug_assert_fn_err_ne_expr.html)
///
#[macro_export]
macro_rules! assert_fn_err_ne_expr_as_result {
    ...
    ($a_function:path, $b_expr:expr) => ({
        ...
        if !a_is_err {
            Err(format!(
                concat!(
                    "assertion failed: `assert_fn_err_ne_expr!(left_function, right_expr)`\n",
                    " left_function label: `{}`,\n",
                    "    right_expr label: `{}`,\n",
                    "    right_expr debug: `{:?}`,\n",
                    "         left result: `{:?}`",
    ...
    mod assert_fn_err_ne_expr_as_result {
        ...
        mod arity_1 {
            ...
            fn test_ne() {
                let a: i8 = 1;
                let b: i8 = 2;
                let x = assert_fn_err_ne_expr_as_result!(f, a, b);
                assert_eq!(x, Ok(()));
            }
        ...
        mod arity_0 {
            ...
            fn test_ne() {
                let b: i8 = 2;
                let x = assert_fn_err_ne_expr_as_result!(f, b);
                assert_eq!(x, Ok(()));
            }
