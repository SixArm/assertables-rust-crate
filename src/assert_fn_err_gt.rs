...
mod test_x_result {
    ...
    fn test_assert_fn_err_gt_as_result_x_success() {
        let a: i32 = 20;
        let b: i32 = 10;
        let x = assert_fn_err_gt_as_result!(example_digit_to_string, a, example_digit_to_string, b);
        assert!(x.is_ok());
        assert_eq!(x, Ok(()));
    }
...
macro_rules! assert_fn_err_gt {
    ($a_function:path, $a_input:expr, $b_expr:expr $(,)?) => ({
        match assert_fn_err_gt_as_result!($a_function, $a_input, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a function err() is greater than another.
///
/// This macro provides the same statements as [`assert_fn_err_gt`],
