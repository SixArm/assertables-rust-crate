...
/// # #[macro_use] extern crate assertables;
/// # fn main() {
/// let x = assure_io_gt!(1, 2); //-> Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "…");
/// assert!(x.is_err());
/// assert_eq!(
///     x.unwrap_err().get_ref().unwrap().to_string(),
///     "assurance failed: `assure_io_gt!(left, right)`\n  left: `1`,\n right: `2`"
/// );
/// # }
/// ```
///
                ...
                if (left_val > right_val) {
                    ...
                    Err(
                        std::io::Error::new(
                            ...
                            format!("assurance failed: `assure_io_gt!(left, right)`\n  left: `{:?}`,\n right: `{:?}`", $left, $right)
                        )
                    )
                }
            }
