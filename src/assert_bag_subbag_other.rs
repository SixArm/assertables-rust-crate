...
mod test_x_result {
    ...
    #[test]
    fn test_assert_bag_subbag_other_as_result_x_success() {
        let a = [1, 1];
        let b = [1, 1, 1];
    ...
    #[test]
    fn test_assert_bag_subbag_other_as_result_x_failure_because_key_is_missing() {
        let a = [1, 1];
        let b = [2, 2];
        let x = assert_bag_subbag_other_as_result!(&a, &b);
        assert!(x.is_err());
    ...
    #[test]
    fn test_assert_bag_subbag_other_as_result_x_failure_because_val_count_is_excessive() {
        let a = [1, 1, 1];
        let b = [1, 1];
        let x = assert_bag_subbag_other_as_result!(&a, &b);
        assert!(x.is_err());
...
/// let a = [1, 1];
/// let b = [2, 2];
/// assert_bag_subbag_other!(&a, &b, "message");
/// //-> panic!
/// });
/// assert!(result.is_err());
/// let actual = result.unwrap_err().downcast::<String>().unwrap().to_string();
...
macro_rules! assert_bag_subbag_other {
    ($a:expr, $b:expr $(,)?) => ({
        match assert_bag_subbag_other_as_result!($a, $b) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
macro_rules! debug_assert_bag_subbag_other {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_bag_subbag_other!($($arg)*);
        }
    };
}
