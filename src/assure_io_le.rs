...
mod tests {
    ...
    fn test_assure_io_le_x_arity_3_return_ok() {
        assert_eq!(
            assure_io_le!(1, 2, "message").unwrap(),
            true
        );
    } 
    ...
    fn test_assure_io_le_x_arity_2_return_err() {
        assert_eq!(
            assure_io_le!(2, 1).unwrap_err().get_ref().unwrap().to_string(), 
    ...
    fn test_assure_io_le_x_arity_3_return_err() {
        assert_eq!(
            assure_io_le!(2, 1, "message").unwrap_err().get_ref().unwrap().to_string(),
            "message"
        );
    } 
