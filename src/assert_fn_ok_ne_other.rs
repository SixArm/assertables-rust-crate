...
/// let expect = concat!(
///     "assertion failed: `assert_fn_ok_ne_other!(pair_function, left_input, right_input)`\n",
...
/// * [`assert_fn_ok_ne_other_as_result`]
/// * [`debug_assert_fn_ok_ne_other`]
///
#[macro_export]
...
mod test_x_result {
    ...
    #[test]
    fn test_assert_fn_ok_ne_other_as_result_x_success() {
        let a: i32 = 1;
        let b: i32 = 2;
        let x = assert_fn_ok_ne_other_as_result!(example_digit_to_string, a, b);
        assert!(x.is_ok());
    ...
    #[test]
    fn test_assert_fn_ok_ne_other_as_result_x_failure() {
        let a: i32 = 1;
        let b: i32 = 1;
        let x = assert_fn_ok_ne_other_as_result!(example_digit_to_string, a, b);
        assert!(x.is_err());
...
macro_rules! assert_fn_ok_ne_other {
    ($function:path, $a_input:expr, $b_input:expr $(,)?) => ({
        match assert_fn_ok_ne_other_as_result!($function, $a_input, $b_input) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a function ok() is not equal to another.
///
/// This macro provides the same statements as [`assert_fn_ok_ne_other`],
