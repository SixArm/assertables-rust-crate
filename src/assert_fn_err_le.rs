...
macro_rules! assert_fn_err_le_as_result {
    ($a_function:path, $a_input:expr, $b_function:path, $b_input:expr $(,)?) => ({
        ...
        if !a_is_err || !b_is_err {
            Err(format!(
                concat!(
                    "assertion failed: `assert_fn_err_le!(left_function, left_input, right_function, right_input)`\n",
                    "  left_function label: `{}`,\n",
                    "     left_input label: `{}`,\n",
                    "     left_input debug: `{:?}`,\n",
...
mod test_x_result {
    ...
    #[test]
    fn test_assert_fn_err_le_as_result_x_success_because_lt() {
        let a: i32 = 10;
        let b: i32 = 20;
        let x = assert_fn_err_le_as_result!(example_digit_to_string, a, example_digit_to_string, b);
        assert!(x.is_ok());
        assert_eq!(x, Ok(()));
    }

    #[test]
    fn test_assert_fn_err_le_as_result_x_success_because_eq() {
        let a: i32 = 10;
        let b: i32 = 10;
        let x = assert_fn_err_le_as_result!(example_digit_to_string, a, example_digit_to_string, b);
        assert!(x.is_ok());
    ...
    #[test]
    fn test_assert_fn_err_le_as_result_x_failure_because_gt() {
        let a: i32 = 20;
        let b: i32 = 10;
        let x = assert_fn_err_le_as_result!(example_digit_to_string, a, example_digit_to_string, b);
        assert!(x.is_err());
...
/// // Panic with error message
/// let result = panic::catch_unwind(|| {
/// assert_fn_err_le!(example_digit_to_string, a, b, "message");
/// //-> panic!
/// });
/// assert!(result.is_err());
/// let actual = result.unwrap_err().downcast::<String>().unwrap().to_string();
...
macro_rules! assert_fn_err_le {
    ($a_function:path, $a_input:expr, $b_expr:expr $(,)?) => ({
        match assert_fn_err_le_as_result!($a_function, $a_input, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a function err() is less than or equal to another.
///
/// This macro provides the same statements as [`assert_fn_err_le`],
