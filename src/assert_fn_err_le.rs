...
/// # Module macros
///
/// * [`assert_fn_err_le`](macro.assert_fn_err_le.html)
/// * [`assert_fn_err_le_as_result`](macro.assert_fn_err_le_as_result.html)
/// * [`debug_assert_fn_err_le`](macro.debug_assert_fn_err_le.html)
///
#[macro_export]
macro_rules! assert_fn_err_le_as_result {
    ...
    ($a_function:path, $b_function:path) => ({
        ...
        if !a_is_err || !b_is_err {
            Err(format!(
                concat!(
                    "assertion failed: `assert_fn_err_le!(left_function, right_function)`\n",
                    "  left_function label: `{}`,\n",
                    " right_function label: `{}`,\n",
                    "                 left: `{:?}`,\n",
                    "                right: `{:?}`"
...
mod tests {
    ...
    mod assert_fn_err_le_as_result {
        ...
        mod arity_1 {
            ...
            fn test_lt() {
                let a: i8 = 1;
                let b: i8 = 2;
                let x = assert_fn_err_le_as_result!(f, a, g, b);
                assert_eq!(x, Ok(()));
            }
        ...
        mod arity_0 {
            ...
            #[test]
            fn test_lt() {
                let x = assert_fn_err_le_as_result!(f, g);
                assert_eq!(x, Ok(()));
            }

            #[test]
            fn test_eq() {
                let x = assert_fn_err_le_as_result!(f, f);
                assert_eq!(x, Ok(()));
            }

            #[test]
            fn test_gt() {
                let x = assert_fn_err_le_as_result!(g, f);
                assert!(x.is_err());
                assert_eq!(
                    x.unwrap_err(),
                    concat!(
...
macro_rules! assert_fn_err_le {
    ...
    ($a_function:path, $b_function:path) => ({
        match assert_fn_err_le_as_result!($a_function, $b_function) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
