...
mod tests {
    ...
    #[test]
    fn test_assert_io_read_to_string_ge_as_result_x_success() {
        let mut a = "bravo".as_bytes();
        let mut b = "alfa".as_bytes();
        let x = assert_io_read_to_string_ge_as_result!(a, b);
        assert_eq!(x, Ok(()));
    }

    #[test]
    fn test_assert_io_read_to_string_ge_as_result_x_failure() {
...
macro_rules! assert_io_read_to_string_ge {
    ($a_reader:expr, $b_reader:expr $(,)?) => ({
        match assert_io_read_to_string_ge_as_result!($a_reader, $b_reader) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
