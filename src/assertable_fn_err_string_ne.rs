...
///
/// # fn main() {
/// let x = assertable_fn_err_string_ne!(example_digit_to_string, 10, 20);
/// //-> Ok(())
/// assert_eq!(x.unwrap(), ());
///
/// let x = assertable_fn_err_string_ne!(example_digit_to_string, 10, 10);
/// //-> Err("…")
/// // assertable failed: `assertable_fn_err_string_ne!(function, left, right)`
/// //      function: `\"example_digit_to_string\"`,
/// //    left input: `10`,
/// //   right input: `10`,
/// //   left output: `\"10 is out of range\"`,
/// //  right output: `\"10 is out of range\"`
/// assert_eq!(x.unwrap_err(), "assertable failed: `assertable_fn_err_string_ne!(function, left, right)`\n     function: `\"example_digit_to_string\"`,\n   left input: `10`,\n  right input: `10`,\n  left is err: `true`,\n right is err: `true`,\n  left output: `\"10 is out of range\"`,\n right output: `\"10 is out of range\"`".to_string());
/// # }
/// ```
///
    ...
    ($function:path, $a:expr, $b:expr $(,)?) => ({
        ...
        if !a_is_err || !b_is_err {
            ...
            if a_string != b_string {
                ...
                Err(format!("assertable failed: `assertable_fn_err_string_ne!(function, left, right)`\n     function: `{:?}`,\n   left input: `{:?}`,\n  right input: `{:?}`,\n  left is err: `{:?}`,\n right is err: `{:?}`,\n  left output: `{:?}`,\n right output: `{:?}`", stringify!($function), $a, $b, a_is_err, b_is_err, a_string, b_string))
            }
        }
    });
...
mod tests {
    ...
    fn test_assertable_fn_err_string_ne_x_arity_2_success() {
        let a = 10;
        let b = 20;
        let x = assertable_fn_err_string_ne!(example_digit_to_string, a, b);
        assert_eq!(
            x.unwrap(),
            ()
        );
    ...
    fn test_assertable_fn_err_string_ne_x_arity_3_success() {
        let a = 10;
        let b = 20;
        let x = assertable_fn_err_string_ne!(example_digit_to_string, a, b, "message");
        assert_eq!(
            x.unwrap(),
            ()
        );
