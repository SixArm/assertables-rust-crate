                        ...
                        if a.gt($b_expr) {
                            ...
                            Err(format!(
                                concat!(
                                    ...
                                    "https://docs.rs/assertables/8.16.0/assertables/macro.assert_program_args_stdout_gt_expr.html\n",
                                    " a_program label: `{}`,\n",
                                    " a_program debug: `{:?}`,\n",
                                    "    a_args label: `{}`,\n",
                                    "    a_args debug: `{:?}`,\n",
...
macro_rules! assert_program_args_stdout_gt_expr {
    ($a_program:expr, $a_args:expr, $b_expr:expr $(,)?) => {{
        match $crate::assert_program_args_stdout_gt_expr_as_result!($a_program, $a_args, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    }};
