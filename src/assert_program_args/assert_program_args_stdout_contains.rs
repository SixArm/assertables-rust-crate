                ...
                if a_output.is_err() {
                    Err(format!(
                        concat!(
                            ...
                            "https://docs.rs/assertables/", env!("CARGO_PKG_VERSION"), "/assertables/macro.assert_program_args_stdout_contains.html\n",
                            " a_program label: `{}`,\n",
                            " a_program debug: `{:?}`,\n",
                            "    a_args label: `{}`,\n",
                            "    a_args debug: `{:?}`,\n",
...
/// # });
/// // assertion failed: `assert_program_args_stdout_contains!(a_program, a_args, containee)`
/// // https://docs.rs/assertables/8.18.0/assertables/macro.assert_program_args_stdout_contains.html
...
/// Assert a command (built with program and args) stdout string contains a given containee.
///
/// This macro provides the same statements as [`assert_program_args_stdout_contains`](macro.assert_program_args_stdout_contains.html),
...
macro_rules! debug_assert_program_args_stdout_contains {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_program_args_stdout_contains!($($arg)*);
        }
    };
}
