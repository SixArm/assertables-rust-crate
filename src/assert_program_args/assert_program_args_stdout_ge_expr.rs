...
//! assert_program_args_stdout_ge_expr!(&program, &args, &bytes);
//! # }
//! ```
//!
//! # Module macros
//!
//! * [`assert_program_args_stdout_ge_expr`](macro@crate::assert_program_args_stdout_ge_expr)
//! * [`assert_program_args_stdout_ge_expr_as_result`](macro@crate::assert_program_args_stdout_ge_expr_as_result)
//! * [`debug_assert_program_args_stdout_ge_expr`](macro@crate::debug_assert_program_args_stdout_ge_expr)
                        ...
                        if a.ge($b_expr) {
                            ...
                            Err(format!(
                                concat!(
                                    ...
                                    "https://docs.rs/assertables/8.14.0/assertables/macro.assert_program_args_stdout_ge_expr.html\n",
                                    " a_program label: `{}`,\n",
                                    " a_program debug: `{:?}`,\n",
                                    "    a_args label: `{}`,\n",
                                    "    a_args debug: `{:?}`,\n",
...
mod tests {
    ...
    fn test_assert_program_args_stdout_ge_expr_as_result_x_eq_success_because_gt() {
        ...
        let a_args = ["%s", "alfa"];
        let b = vec![b'a', b'a'];
        let result = assert_program_args_stdout_ge_expr_as_result!(&a_program, &a_args, &b);
...
macro_rules! assert_program_args_stdout_ge_expr {
    ($a_program:expr, $a_args:expr, $b_expr:expr $(,)?) => {{
        match $crate::assert_program_args_stdout_ge_expr_as_result!($a_program, $a_args, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    }};
...
/// (program1 + args1 ⇒ command ⇒ stdout ⇒ string) ≥ (expr into string)
///
/// This macro provides the same statements as [`assert_program_args_stdout_ge_expr`](macro.assert_program_args_stdout_ge_expr.html),
