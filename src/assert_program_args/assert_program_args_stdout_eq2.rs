...
//! # Module macros
//!
//! * [`assert_program_args_stdout_eq2`](macro@crate::assert_program_args_stdout_eq2)
//! * [`assert_program_args_stdout_eq2_as_result`](macro@crate::assert_program_args_stdout_eq2_as_result)
//! * [`debug_assert_program_args_stdout_eq2`](macro@crate::debug_assert_program_args_stdout_eq2)
...
///
#[macro_export]
macro_rules! assert_program_args_stdout_eq2_as_result {
                    ...
                    (Ok(a_output), Ok(b_output)) => {
                        ...
                        if a.eq(&b) {
                            ...
                            Err(
                                format!(
                                    concat!(
                                        "assertion failed: `assert_program_args_stdout_eq2!(a_program, a_args, b_program, b_args)`\n",
                                        "https://docs.rs/assertables/9.0.0/assertables/macro.assert_program_args_stdout_eq2.html\n",
                                        " a_program label: `{}`,\n",
                                        " a_program debug: `{:?}`,\n",
                                        "    a_args label: `{}`,\n",
                                        "    a_args debug: `{:?}`,\n",
...
mod tests {
    ...
    fn test_assert_program_args_stdout_eq_as_result_x_success() {
        ...
        let b_args = ["%s%s%s%s", "a", "l", "f", "a"];
        let result =
            assert_program_args_stdout_eq2_as_result!(&a_program, &a_args, &b_program, &b_args);
        assert_eq!(
            result.unwrap(),
...
///
#[macro_export]
macro_rules! assert_program_args_stdout_eq2 {
    ($a_program:expr, $a_args:expr, $b_program:expr, $b_args:expr $(,)?) => {{
        match $crate::assert_program_args_stdout_eq2_as_result!($a_program, $a_args, $b_program, $b_args) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    }};
...
/// (program1 + args1 ⇒ command ⇒ stdout) = (program2 + args2 ⇒ command ⇒ stdout)
///
/// This macro provides the same statements as [`assert_program_args_stdout_eq2`](macro.assert_program_args_stdout_eq2.html),
...
///
#[macro_export]
macro_rules! debug_assert_program_args_stdout_eq2 {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_program_args_stdout_eq2!($($arg)*);
        }
    };
}
