...
//! assert_program_args_stdout_eq!(&program, &args, bytes);
//! # }
//! ```
//!
//! # Module macros
                    ...
                    Ok(a_output) => {
                        ...
                        if a.eq(&$b_expr) {
                            ...
                            Err(
                                format!(
                                    concat!(
                                        "assertion failed: `assert_program_args_stdout_eq!(a_program, a_args, b_expr)`\n",
                                        "https://docs.rs/assertables/9.0.0/assertables/macro.assert_program_args_stdout_eq.html\n",
                                        " a_program label: `{}`,\n",
                                        " a_program debug: `{:?}`,\n",
                                        "    a_args label: `{}`,\n",
                                        "    a_args debug: `{:?}`,\n",
...
mod tests {
    ...
    fn test_assert_program_args_stdout_eq_expr_as_result_x_success() {
        ...
        let result = assert_program_args_stdout_eq_as_result!(&a_program, &a_args, b);
        assert_eq!(
            result.unwrap(),
...
macro_rules! assert_program_args_stdout_eq {
    ($a_program:expr, $a_args:expr, $b_expr:expr $(,)?) => {{
        match $crate::assert_program_args_stdout_eq_as_result!($a_program, $a_args, $b_expr) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    }};
