...
//! assert_program_args_stdout_le_expr!(&program, &args, &bytes);
//! # }
//! ```
//!
//! # Module macros
//!
//! * [`assert_program_args_stdout_le_expr`](macro@crate::assert_program_args_stdout_le_expr)
//! * [`assert_program_args_stdout_le_expr_as_result`](macro@crate::assert_program_args_stdout_le_expr_as_result)
//! * [`debug_assert_program_args_stdout_le_expr`](macro@crate::debug_assert_program_args_stdout_le_expr)
                ...
                match assert_program_args_impl_prep!(a_program, a_args) {
                    Ok(a_output) => {
                        ...
                        if a.le($b_expr) {
                            ...
                            Err(format!(
                                concat!(
                                    "assertion failed: `assert_program_args_stdout_le_expr!(a_program, a_args, b_expr)`\n",
                                    "https://docs.rs/assertables/8.14.0/assertables/macro.assert_program_args_stdout_le_expr.html\n",
                                    " a_program label: `{}`,\n",
                                    " a_program debug: `{:?}`,\n",
                                    "    a_args label: `{}`,\n",
                                    "    a_args debug: `{:?}`,\n",
...
mod tests {
    ...
    fn test_assert_program_args_stdout_le_expr_as_result_x_success_because_lt() {
        ...
        let a_args = ["%s", "alfa"];
        let b = vec![b'z', b'z'];
        let result = assert_program_args_stdout_le_expr_as_result!(&a_program, &a_args, &b);
...
macro_rules! assert_program_args_stdout_le_expr {
    ($a_program:expr, $a_args:expr, $b_expr:expr $(,)?) => {{
        match $crate::assert_program_args_stdout_le_expr_as_result!($a_program, $a_args, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    }};
...
/// (program1 + args1 ⇒ command ⇒ stdout) ≤ (expr into string)
///
/// This macro provides the same statements as [`assert_program_args_stdout_le_expr`](macro.assert_program_args_stdout_le_expr.html),
...
macro_rules! debug_assert_program_args_stdout_le_expr {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_program_args_stdout_le_expr!($($arg)*);
        }
    };
}
