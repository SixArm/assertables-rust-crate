                            ...
                            Err(
                                format!(
                                    concat!(
                                        ...
                                        "https://docs.rs/assertables/9.4.0/assertables/macro.assert_program_args_stderr_gt_x.html\n",
                                        " a_program label: `{}`,\n",
                                        " a_program debug: `{:?}`,\n",
                                        "    a_args label: `{}`,\n",
                                        "    a_args debug: `{:?}`,\n",
...
mod tests {
    ...
    #[test]
    fn test_assert_program_args_stderr_gt_x_expr_as_result_x_success() {
        let a_program = "bin/printf-stderr";
        let a_args = ["%s", "alfa"];
        let b = vec![b'a', b'a'];
        let result = assert_program_args_stderr_gt_x_as_result!(&a_program, &a_args, b);
        assert_eq!(result.unwrap(), vec![b'a', b'l', b'f', b'a']);
    }

    #[test]
    fn test_assert_program_args_stderr_gt_x_expr_as_result_x_failure() {
        let a_program = "bin/printf-stderr";
        let a_args = ["%s", "alfa"];
        let b = vec![b'z', b'z'];
        let result = assert_program_args_stderr_gt_x_as_result!(&a_program, &a_args, b);
