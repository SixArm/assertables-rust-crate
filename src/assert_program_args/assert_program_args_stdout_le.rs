...
//! assert_program_args_stdout_le!(&program, &args, bytes);
//! # }
//! ```
//!
//! # Module macros
                    ...
                    Ok(a_output) => {
                        ...
                        if a.le(&$b_expr) {
                            ...
                            Err(
                                format!(
                                    concat!(
                                        "assertion failed: `assert_program_args_stdout_le!(a_program, a_args, b_expr)`\n",
...
macro_rules! assert_program_args_stdout_le {
    ($a_program:expr, $a_args:expr, $b_expr:expr $(,)?) => {{
        match $crate::assert_program_args_stdout_le_as_result!($a_program, $a_args, $b_expr) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    }};
