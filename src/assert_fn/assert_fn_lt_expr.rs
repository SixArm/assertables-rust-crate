    ...
    mod assert_fn_lt_expr_as_result {
        ...
        mod arity_1 {
            ...
            fn test_lt() {
                let a: i8 = 1;
                let b: i8 = 2;
                let result = assert_fn_lt_expr_as_result!(f, a, b);
                assert_eq!(result, Ok(()));
            }
        ...
        mod arity_0 {
            ...
            fn test_lt() {
                let b: i8 = 2;
                let result = assert_fn_lt_expr_as_result!(f, b);
                assert_eq!(result, Ok(()));
            }
...
/// # });
/// // assertion failed: `assert_fn_lt_expr!(a_function, a_param, b_expr)`
/// // https://docs.rs/assertables/8.16.0/assertables/macro.assert_fn_lt_expr.html
...
macro_rules! assert_fn_lt_expr {
    ...
    ($a_function:path, $a_param:expr, $b_expr:expr $(,)?) => {{
        match $crate::assert_fn_lt_expr_as_result!($a_function, $a_param, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    }};
    ...
    ($a_function:path, $b_expr:expr $(,)?) => {{
        match $crate::assert_fn_lt_expr_as_result!($a_function, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    }};
