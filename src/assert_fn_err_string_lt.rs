...
/// //-> panic!("…")
/// // assertion failed: `assert_fn_err_string_lt!(function, left, right)`
...
macro_rules! assert_fn_err_string_lt {
    ($function:path, $a:expr, $b:expr $(,)?) => ({
        ...
        if !a_is_err || !b_is_err {
            panic!("assertion failed: `assert_fn_err_lt_string!(function, left, right)`\n     function: `{:?}`,\n   left input: `{:?}`,\n  right input: `{:?}`,\n  left is err: `{:?}`,\n right is err: `{:?}`", stringify!($function), $a, $b, a_is_err, b_is_err);
        } else {
...
mod tests {
    ...
    fn test_assert_fn_err_string_lt_x_arity_2_lt_success() {
        let a = 10;
        let b = 20;
        let x = assert_fn_err_string_lt!(example_digit_to_string, a, b);
        assert_eq!(x, ());
    }

    #[test]
    #[should_panic (expected = "assertion failed: `assert_fn_err_string_lt!(function, left, right)`\n     function: `\"example_digit_to_string\"`,\n   left input: `10`,\n  right input: `10`,\n  left is err: `true`,\n right is err: `true`,\n  left output: `\"10 is out of range\"`,\n right output: `\"10 is out of range\"`")]
    fn test_assert_fn_err_string_lt_x_arity_2_eq_failure() {
        let a = 10;
        let b = 10;
        let _x = assert_fn_err_string_lt!(example_digit_to_string, a, b);
    }
    ...
    fn test_assert_fn_err_string_lt_x_arity_3_lt_success() {
        let a = 10;
        let b = 20;
        let x = assert_fn_err_string_lt!(example_digit_to_string, a, b, "message");
        assert_eq!(x, ());
    }
    ...
    fn test_assert_fn_err_string_lt_x_arity_3_eq_failure() {
        let a = 10;
        let b = 10;
        let _x = assert_fn_err_string_lt!(example_digit_to_string, a, b, "message");
    }
