...
/// Assert a command (built with program and args) stdout string is a match to a regex.
///
/// This macro provides the same statements as [`assert_program_args_stdout_matches`],
...
/// 
#[macro_export]
macro_rules! debug_assert_program_args_stdout_matches {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_program_args_stdout_matches!($($arg)*);
        }
    };
}
