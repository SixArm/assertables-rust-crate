...
/// # fn main() {
/// let x = assertable_fn_err_string_ge!(f, 2, 1);
/// //-> Ok(())
/// assert_eq!(x.unwrap(), ());
///
/// let x = assertable_fn_err_string_ge!(f, 1, 2);
/// //-> Err("…")
/// // assertable failed: `assertable_fn_err_string_ge!(fn, left, right)`
/// //    left input: `1`,
/// //   right input: `2`,
/// //   left output: `\"1\"`,
/// //  right output: `\"2\"
/// assert_eq!(x.unwrap_err(), "assertable failed: `assertable_fn_err_string_ge!(fn, left, right)`\n   left input: `1`,\n  right input: `2`,\n  left output: `\"1\"`,\n right output: `\"2\"`".to_string());
/// # }
/// ```
///
...
macro_rules! assertable_fn_err_string_ge {
    ($function:path, $left:expr, $right:expr $(,)?) => ({
        ...
        if !left.is_err() || !right.is_err() {
            Err(format!("assertable failed: `assertable_fn_err_string_ge!(fn, left, right)`\n   left input: `{:?}`,\n  right input: `{:?}`\n  left output is_err(): `{:?}`,\n right output is_err(): `{:?}`", $left, $right, left.is_err(), right.is_err()))
        } else {
...
mod tests {
    ...
    fn test_assertable_fn_err_string_ge_x_arity_2_gt_success() {
        let a = 2;
        let b = 1;
        let x = assertable_fn_err_string_ge!(f, a, b);
        assert!(x.is_ok());
    }
    ...
    fn test_assertable_fn_err_string_ge_x_arity_3_gt_success() {
        let a = 2;
        let b = 1;
        let x = assertable_fn_err_string_ge!(f, a, b, "message");
        assert!(x.is_ok());
    }
