        ...
        match (&$a, &$b) {
            (a_val, b_val) => {
                ...
                if a_set != b_set {
                    ...
                    Err(format!(
                        concat!(
                            "assertion failed: `assert_set_ne_other!(left_set, right_set)`\n",
                            "  left_set label: `{}`,\n",
                            "  left_set debug: `{:?}`,\n",
                            " right_set label: `{}`,\n",
                            " right_set debug: `{:?}`,\n",
...
mod test_x_result {
    ...
    #[test]
    fn test_assert_set_ne_other_as_result_x_success() {
        let a = [1, 2];
        let b = [3, 4];
        let x = assert_set_ne_other_as_result!(&a, &b);
        assert!(x.is_ok());
        assert_eq!(x, Ok(()));
    }

    #[test]
    fn test_assert_set_ne_other_as_result_x_failure() {
        let a = [1, 2];
        let b = [1, 2];
        let x = assert_set_ne_other_as_result!(&a, &b);
        assert!(x.is_err());
...
macro_rules! assert_set_ne_other {
    ($a:expr, $b:expr $(,)?) => ({
        match assert_set_ne_other_as_result!($a, $b) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a set is not equal to another.
///
/// This macro provides the same statements as [`assert_set_ne_other`],
...
///
#[macro_export]
macro_rules! debug_assert_set_ne_other {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_set_ne_other!($($arg)*);
        }
    };
}
