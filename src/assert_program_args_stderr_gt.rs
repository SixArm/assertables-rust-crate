...
/// # Module macros
///
/// * [`assert_program_args_stderr_gt`](macro.assert_program_args_stderr_gt.html)
/// * [`assert_program_args_stderr_gt_as_result`](macro.assert_program_args_stderr_gt_as_result.html)
/// * [`debug_assert_program_args_stderr_gt`](macro.debug_assert_program_args_stderr_gt.html)
///
#[macro_export]
...
mod tests {
    ...
    #[test]
    fn test_assert_program_args_stderr_gt_as_result_x_success() {
        let a_program = "bin/printf-stderr";
        let a_args = ["%s", "hello"];
        let b_program = "bin/printf-stderr";
        let b_args = ["%s", "hallo"];
        let x = assert_program_args_stderr_gt_as_result!(&a_program, &a_args, &b_program, &b_args);
        assert_eq!(x.unwrap(), ());
    }
...
macro_rules! assert_program_args_stderr_gt {
    ($a_program:expr, $a_args:expr, $b_program:expr, $b_args:expr $(,)?) => ({
        match assert_program_args_stderr_gt_as_result!($a_program, $a_args, $b_program, $b_args) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
///
/// This macro provides the same statements as [`assert_program_args_stderr_gt`],
