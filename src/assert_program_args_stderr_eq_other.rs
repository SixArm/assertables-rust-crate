...
/// Assert a command (built with program and args) stderr string is equal to another.
///
/// This macro provides the same statements as [`assert_program_args_stderr_eq_other`],
...
/// 
#[macro_export]
macro_rules! debug_assert_program_args_stderr_eq_other {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_program_args_stderr_eq_other!($($arg)*);
        }
    };
}
