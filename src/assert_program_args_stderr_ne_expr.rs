...
/// * [`debug_assert_program_args_stderr_ne_expr`]
///
#[macro_export]
...
mod tests {
    ...
    fn test_assert_program_args_stderr_ne_expr_as_result_x_success() {
        ...
        let x = assert_program_args_stderr_ne_expr_as_result!(&a_program, &a_args, b);
        assert_eq!(x.unwrap(), ());
    }
...
macro_rules! assert_program_args_stderr_ne_expr {
    ($a_program:expr, $a_args:expr, $b_expr:expr $(,)?) => ({
        match assert_program_args_stderr_ne_expr_as_result!($a_program, $a_args, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a command (built with program and args) stderr string is not equal to an expression.
///
/// This macro provides the same statements as [`assert_program_args_stderr_ne_expr`],
...
macro_rules! debug_assert_program_args_stderr_ne_expr {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_program_args_stderr_ne_expr!($($arg)*);
        }
    };
}
