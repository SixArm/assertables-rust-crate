...
/// let x = assert_read_to_string_lt_other_as_result!(a, b);
/// //-> Ok(())
/// assert_eq!(x, Ok(()));
/// let actual = x.unwrap();
/// let expect = ();
...
/// * [`assert_read_to_string_lt_other_as_result`]
/// * [`debug_assert_read_to_string_lt_other`]
///
#[macro_export]
...
mod test_x_result {
    ...
    #[test]
    fn test_assert_read_to_string_lt_other_as_result_x_success() {
        let mut a = "alpha".as_bytes();
        let mut b = "bravo".as_bytes();
        let x = assert_read_to_string_lt_other_as_result!(a, b);
        assert!(x.is_ok());
    ...
    #[test]
    fn test_assert_read_to_string_lt_other_as_result_x_failure() {
        let mut a = "bravo".as_bytes();
        let mut b = "alpha".as_bytes();
        let x = assert_read_to_string_lt_other_as_result!(a, b);
        assert!(x.is_err());
...
/// let mut a = "alpha".as_bytes();
/// let mut b = "bravo".as_bytes();
/// assert_read_to_string_lt_other!(a, b);
/// //-> ()
///
...
///
#[macro_export]
macro_rules! assert_read_to_string_lt_other {
    ($a_reader:expr, $b_reader:expr $(,)?) => ({
        match assert_read_to_string_lt_other_as_result!($a_reader, $b_reader) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a std::io::Read read_to_string() value is less than another.
///
/// This macro provides the same statements as [`assert_read_to_string_lt_other`],
...
/// 
#[macro_export]
macro_rules! debug_assert_read_to_string_lt_other {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_read_to_string_lt_other!($($arg)*);
        }
    };
}
