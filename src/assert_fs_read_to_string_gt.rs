...
///
/// * [`read_to_string_gt`]
/// * [`read_to_string_gt_as_result`]
/// * [`debug_read_to_string_gt`]
///
#[macro_export]
...
mod tests {
    ...
    fn test_read_to_string_gt_as_result_x_success() {
        ...
        let x = assert_fs_read_to_string_gt_as_result!(&a, &b);
        assert_eq!(x, Ok(()));
    }
...
macro_rules! assert_fs_read_to_string_gt {
    ($a_path:expr, $b_path:expr $(,)?) => ({
        match assert_fs_read_to_string_gt_as_result!($a_path, $b_path) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a std::fs::read_to_string() value is greater than another.
///
/// This macro provides the same statements as [`read_to_string_gt`],
