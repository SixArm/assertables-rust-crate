...
mod test_x_result {
    ...
    fn test_assert_command_stdout_is_match_as_result_x_success() {
        ...
        let x = assert_command_stdout_is_match_as_result!(a, b);
        assert_eq!(x.unwrap(), ());
    }
...
macro_rules! assert_command_stdout_is_match {
    ($a_command:expr, $b_matcher:expr $(,)?) => ({
        match assert_command_stdout_is_match_as_result!($a_command, $b_matcher) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a command stdout string is a match to a regex.
///
/// This macro provides the same statements as [`assert_command_stdout_is_match`],
