...
//! assert_command_stdout_le!(command, bytes);
//! # }
//! ```
//!
//! # Module macros
                    ...
                    Ok(a) => {
                        ...
                        if a.le(&$b_expr) {
                            ...
                            Err(
                                format!(
                                    concat!(
                                        "assertion failed: `assert_command_stdout_le!(command, expr)`\n",
                                        "https://docs.rs/assertables/9.0.0/assertables/macro.assert_command_stdout_le.html\n",
                                        " command label: `{}`,\n",
                                        " command debug: `{:?}`,\n",
                                        "    expr label: `{}`,\n",
                                        "    expr debug: `{:?}`,\n",
...
macro_rules! assert_command_stdout_le {
    ($a_command:expr, $b_expr:expr $(,)?) => {{
        match $crate::assert_command_stdout_le_as_result!($a_command, $b_expr) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    }};
