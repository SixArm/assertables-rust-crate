...
//! assert_command_stdout_ne!(command, bytes);
//! # }
//! ```
//!
//! # Module macros
                    ...
                    Ok(a) => {
                        ...
                        if a.ne(&$b_expr) {
                            ...
                            Err(
                                format!(
                                    concat!(
                                        "assertion failed: `assert_command_stdout_ne!(command, expr)`\n",
                                        "https://docs.rs/assertables/9.0.0/assertables/macro.assert_command_stdout_ne.html\n",
                                        " command label: `{}`,\n",
                                        " command debug: `{:?}`,\n",
                                        "    expr label: `{}`,\n",
                                        "    expr debug: `{:?}`,\n",
...
macro_rules! assert_command_stdout_ne {
    ($a_command:expr, $b_expr:expr $(,)?) => {{
        match $crate::assert_command_stdout_ne_as_result!($a_command, $b_expr) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    }};
