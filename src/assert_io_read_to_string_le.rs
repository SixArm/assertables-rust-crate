...
mod tests {
    ...
    fn test_assert_io_read_to_string_le_as_result_x_success() {
        ...
        let x = assert_io_read_to_string_le_as_result!(a, b);
        assert_eq!(x, Ok(()));
    }
...
macro_rules! assert_io_read_to_string_le {
    ($a_reader:expr, $b_reader:expr $(,)?) => ({
        match assert_io_read_to_string_le_as_result!($a_reader, $b_reader) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a std::io::Read read_to_string() value is less than or equal to another.
///
/// This macro provides the same statements as [`assert_io_read_to_string_le`],
