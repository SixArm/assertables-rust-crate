    ...
    mod assert_fn_err_eq_as_result {
        ...
        mod arity_1 {
            ...
            fn test_eq() {
                let a: i8 = 1;
                let b: i8 = 1;
                let x = assert_fn_err_eq_as_result!(f, a, g, b);
                assert_eq!(x, Ok(()));
            }
        ...
        mod arity_0 {
            ...
            #[test]
            fn test_eq() {
                let x = assert_fn_err_eq_as_result!(f, f);
                assert_eq!(x, Ok(()));
            }

            #[test]
            fn test_ne() {
                let x = assert_fn_err_eq_as_result!(f, g);
                assert!(x.is_err());
                assert_eq!(
                    x.unwrap_err(),
                    concat!(
...
macro_rules! assert_fn_err_eq {
    ...
    ($a_function:path, $b_function:path) => ({
        match assert_fn_err_eq_as_result!($a_function, $b_function) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
