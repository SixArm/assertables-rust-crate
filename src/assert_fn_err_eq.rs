...
/// let x = assert_fn_err_eq_as_result!(example_digit_to_string, a, b);
/// //-> Ok(())
/// let actual = x.unwrap();
/// let expect = ();
...
/// let expect = concat!(
///     "assertion failed: `assert_fn_err_eq!(function, left_input, right_expr)`\n",
...
mod test_x_result {
    ...
    #[test]
    fn test_assert_fn_err_eq_as_result_x_arity_2_success() {
        let a: i32 = 10;
        let b = String::from("10 is out of range");
        let x = assert_fn_err_eq_as_result!(example_digit_to_string, a, b);
        assert_eq!(
    ...
    #[test]
    fn test_assert_fn_err_eq_as_result_x_arity_2_failure() {
        let a: i32 = 10;
        let b = String::from("20 is out of range");
        let x = assert_fn_err_eq_as_result!(example_digit_to_string, a, b);
        assert_eq!(
...
macro_rules! assert_fn_err_eq {
    ($function:path, $a_input:expr, $b_expr:expr $(,)?) => ({
        match assert_fn_err_eq_as_result!($function, $a_input, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
mod test_x_panic {
    ...
    #[test]
    #[should_panic (expected = "assertion failed: `assert_fn_err_eq!(function, left_input, right_expr)`\n    function name: `example_digit_to_string`,\n  left input name: `a`,\n  right expr name: `b`,\n       left input: `10`,\n       right expr: `\"20 is out of range\"`,\n      left output: `\"10 is out of range\"`,\n             left: `\"10 is out of range\"`,\n            right: `\"20 is out of range\"`")]
