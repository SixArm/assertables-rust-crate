...
macro_rules! assert_fn_eq_as_result {
    ...
    ($a_function:path, $b_function:path $(,)?) => ({
        ...
        if a_output == b_output {
            ...
            Err(format!(
                concat!(
                    "assertion failed: `assert_fn_eq!(left_function, right_function)`\n",
                    "  left_function label: `{}`,\n",
...
mod test_x_result {
    ...
    #[test]
    fn test_assert_fn_eq_as_result_x_arity_0_x_success_because_eq() {
        let x = assert_fn_eq_as_result!(one, one);
        assert!(x.is_ok());
        assert_eq!(x, Ok(()));
    }

    #[test]
    fn test_assert_fn_eq_as_result_x_arity_0_x_failure_because_ne() {
        let x = assert_fn_eq_as_result!(one, two);
        assert!(x.is_err());
        assert_eq!(
            x.unwrap_err(),
            concat!(
    ...
    #[test]
    fn test_assert_fn_eq_as_result_x_arity_1_x_success_because_eq() {
        let a: i32 = -1;
        let b: i32 = 1;
        let x = assert_fn_eq_as_result!(i32::abs, a, i32::abs, b);
        assert!(x.is_ok());
        assert_eq!(x, Ok(()));
    }

    #[test]
    fn test_assert_fn_eq_as_result_x_arity_1_x_failure_because_ne() {
        let a: i32 = -1;
        let b: i32 = 2;
        let x = assert_fn_eq_as_result!(i32::abs, a, i32::abs, b);
        assert!(x.is_err());
...
macro_rules! assert_fn_eq {
    ...
    ($a_function:path, $b_function:path $(,)?) => ({
        match assert_fn_eq_as_result!($a_function, $b_function) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
    ...
    ($a_function:path, $a_input:expr, $b_function:path, $b_input:expr $(,)?) => ({
        match assert_fn_eq_as_result!($a_function, $a_input, $b_function, $b_input) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
