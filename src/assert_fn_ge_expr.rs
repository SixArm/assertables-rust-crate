...
mod test_x_result {
    ...
    #[test]
    fn test_assert_fn_ge_expr_as_result_x_success_because_gt_expr() {
        let a: i32 = -2;
        let b: i32 = 1;
    ...
    #[test]
    fn test_assert_fn_ge_expr_as_result_x_success_because_eq_expr() {
        let a: i32 = -1;
        let b: i32 = 1;
        let x = assert_fn_ge_expr_as_result!(i32::abs, a, b);
        assert!(x.is_ok());
    ...
    #[test]
    fn test_assert_fn_ge_expr_as_result_x_failure() {
        let a: i32 = -1;
        let b: i32 = 2;
        let x = assert_fn_ge_expr_as_result!(i32::abs, a, b);
        assert!(x.is_err());
...
macro_rules! assert_fn_ge_expr {
    ($function:path, $a_input:expr, $b_expr:expr $(,)?) => ({
        match assert_fn_ge_expr_as_result!($function, $a_input, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
