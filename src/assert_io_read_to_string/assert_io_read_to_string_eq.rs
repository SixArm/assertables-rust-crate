...
//! assert_io_read_to_string_eq!(reader, &value);
//! # }
//! ```
//!
//! # Module macros
                    ...
                    Ok(_a_size) => {
                        ...
                        if a_string == b_string {
                            ...
                            Err(
                                format!(
                                    concat!(
                                        "assertion failed: `assert_io_read_to_string_eq!(a_reader, b_expr)`\n",
...
mod tests {
    ...
    fn test_assert_io_read_to_string_eq_expr_as_result_x_success_because_eq() {
        let mut reader = "alfa".as_bytes();
        let value = String::from("alfa");
        let result = assert_io_read_to_string_eq_as_result!(reader, &value);
        assert_eq!(
            result.unwrap(),
...
macro_rules! assert_io_read_to_string_eq {
    ($a_reader:expr, $b_expr:expr $(,)?) => {{
        match $crate::assert_io_read_to_string_eq_as_result!($a_reader, $b_expr) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    }};
