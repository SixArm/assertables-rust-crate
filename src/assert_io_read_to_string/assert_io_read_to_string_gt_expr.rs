...
//! # Module macros
//!
//! * [`assert_io_read_to_string_gt_expr`](macro@crate::assert_io_read_to_string_gt_expr)
                            ...
                            Err(
                                format!(
                                    concat!(
                                        ...
                                        "https://docs.rs/assertables/9.0.0/assertables/macro.assert_io_read_to_string_gt_expr.html\n",
                                        " a_reader label: `{}`,\n",
                                        " a_reader debug: `{:?}`,\n",
                                        "   b_expr label: `{}`,\n",
                                        "   b_expr debug: `{:?}`,\n",
...
mod tests {
    ...
    #[test]
    fn test_assert_io_read_to_string_gt_expr_as_result_x_success_because_gt() {
        let mut reader = "bravo".as_bytes();
        let value = String::from("alfa");
        let result = assert_io_read_to_string_gt_expr_as_result!(reader, &value);
        assert_eq!(
            result.unwrap(),
    ...
    #[test]
    fn test_assert_io_read_to_string_gt_expr_as_result_x_failure_because_eq() {
    ...
    #[test]
    fn test_assert_io_read_to_string_gt_expr_as_result_x_failure_because_lt() {
        let mut reader = "alfa".as_bytes();
...
macro_rules! assert_io_read_to_string_gt_expr {
    ($a_reader:expr,  $b_expr:expr $(,)?) => {{
        match $crate::assert_io_read_to_string_gt_expr_as_result!($a_reader, $b_expr) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    }};
...
/// (reader.read_to_string(a_string) ⇒ a_string) > (expr ⇒ b_string)
///
/// This macro provides the same statements as [`assert_io_read_to_string_gt_expr`](macro.assert_io_read_to_string_gt_expr.html),
...
macro_rules! debug_assert_io_read_to_string_gt_expr {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_io_read_to_string_gt_expr!($($arg)*);
        }
    };
}
