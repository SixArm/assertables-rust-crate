                            ...
                            Err(
                                format!(
                                    concat!(
                                        ...
                                        "https://docs.rs/assertables/9.6.1/assertables/macro.assert_io_read_to_string_gt_x.html\n",
                                        " a_reader label: `{}`,\n",
                                        " a_reader debug: `{:?}`,\n",
                                        "   b_expr label: `{}`,\n",
                                        "   b_expr debug: `{:?}`,\n",
...
mod test_assert_io_read_to_string_gt_x {
    ...
    fn gt() {
        ...
        let value = String::from("aa");
        for _ in 0..1 {
            let actual = assert_io_read_to_string_gt_x!(reader, &value);
    ...
    fn eq() {
        let result = panic::catch_unwind(|| {
            let mut reader = "alfa".as_bytes();
            let value = String::from("alfa");
            let _actual = assert_io_read_to_string_gt_x!(reader, &value);
        });
        let message = concat!(
            "assertion failed: `assert_io_read_to_string_gt_x!(a_reader, b_expr)`\n",
            "https://docs.rs/assertables/9.6.1/assertables/macro.assert_io_read_to_string_gt_x.html\n",
