...
//! # Module macros
//!
//! * [`assert_io_read_to_string_lt_expr`](macro@crate::assert_io_read_to_string_lt_expr)
//! * [`assert_io_read_to_string_lt_expr_as_result`](macro@crate::assert_io_read_to_string_lt_expr_as_result)
//! * [`debug_assert_io_read_to_string_lt_expr`](macro@crate::debug_assert_io_read_to_string_lt_expr)
                ...
                if let Err(a_err) = a_result {
                    Err(format!(
                        concat!(
                            ...
                            "https://docs.rs/assertables/", env!("CARGO_PKG_VERSION"), "/assertables/macro.assert_io_read_to_string_lt_expr.html\n",
                            " a_reader label: `{}`,\n",
                            " a_reader debug: `{:?}`,\n",
                            "   b_expr label: `{}`,\n",
                            "   b_expr debug: `{:?}`,\n",
...
mod tests {
    ...
    fn test_assert_io_read_to_string_lt_expr_as_result_x_success() {
        let mut reader = "alfa".as_bytes();
        let value = String::from("bravo");
        let result = assert_io_read_to_string_lt_expr_as_result!(reader, &value);
        assert_eq!(result, Ok(()));
    }
...
/// # });
/// // assertion failed: `assert_io_read_to_string_lt_expr!(a_reader, b_expr)`
/// // https://docs.rs/assertables/8.14.0/assertables/macro.assert_io_read_to_string_lt_expr.html
/// //  a_reader label: `reader`,
/// //  a_reader debug: `[]`,
/// //    b_expr label: `&value`,
/// //    b_expr debug: `\"alfa\"`,
...
macro_rules! assert_io_read_to_string_lt_expr {
    ($a_reader:expr,  $b_expr:expr $(,)?) => {{
        match $crate::assert_io_read_to_string_lt_expr_as_result!($a_reader, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    }};
...
/// (reader.read_to_string(a) ⇒ a) <≤> (expr into string)
///
/// This macro provides the same statements as [`assert_io_read_to_string_lt_expr`](macro.assert_io_read_to_string_lt_expr.html),
...
macro_rules! debug_assert_io_read_to_string_lt_expr {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_io_read_to_string_lt_expr!($($arg)*);
        }
    };
}
