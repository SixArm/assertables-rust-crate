...
//! assert_io_read_to_string_lt!(reader, &value);
//! # }
//! ```
//!
//! # Module macros
                    ...
                    Ok(_a_size) => {
                        ...
                        if (a_string < b_string) {
                            ...
                            Err(
                                format!(
                                    concat!(
                                        "assertion failed: `assert_io_read_to_string_lt!(a_reader, b_expr)`\n",
                                        "https://docs.rs/assertables/9.0.0/assertables/macro.assert_io_read_to_string_lt.html\n",
                                        " a_reader label: `{}`,\n",
                                        " a_reader debug: `{:?}`,\n",
                                        "   b_expr label: `{}`,\n",
                                        "   b_expr debug: `{:?}`,\n",
...
mod tests {
    ...
    fn test_assert_io_read_to_string_lt_expr_as_result_x_success() {
        let mut reader = "alfa".as_bytes();
        let value = String::from("bravo");
        let result = assert_io_read_to_string_lt_as_result!(reader, &value);
...
macro_rules! assert_io_read_to_string_lt {
    ($a_reader:expr,  $b_expr:expr $(,)?) => {{
        match $crate::assert_io_read_to_string_lt_as_result!($a_reader, $b_expr) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    }};
