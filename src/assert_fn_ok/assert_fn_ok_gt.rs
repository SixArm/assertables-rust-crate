                    ...
                    Ok(a) => {
                        if a > $b_expr {
                            ...
                            Err(
                                format!(
                                    concat!(
                                        "assertion failed: `assert_fn_ok_gt!(a_function, a_param, b_expr)`\n",
                    ...
                    Ok(a) => {
                        if a > $b_expr {
                            ...
                            Err(
                                format!(
                                    concat!(
                                        "assertion failed: `assert_fn_ok_gt!(a_function, b_expr)`\n",
                                        "https://docs.rs/assertables/9.0.0/assertables/macro.assert_fn_ok_gt.html\n",
                                        " a_function label: `{}`,\n",
                                        "     b_expr label: `{}`,\n",
                                        "     b_expr debug: `{:?}`,\n",
    ...
    mod assert_fn_ok_gt_as_result {
        ...
        mod arity_1 {
            ...
            fn gt() {
                let a: i8 = 2;
                let b: i8 = 1;
                let result = assert_fn_ok_gt_as_result!(f, a, b);
        ...
        mod arity_0 {
            ...
            fn gt() {
                let b: i8 = 0;
                let result = assert_fn_ok_gt_as_result!(f, b);
...
macro_rules! assert_fn_ok_gt {
    ...
    ($a_function:path, $a_param:expr, $b_expr:expr $(,)?) => {{
        match $crate::assert_fn_ok_gt_as_result!($a_function, $a_param, $b_expr) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    }};
    ...
    ($a_function:path, $b_expr:expr $(,)?) => {{
        match $crate::assert_fn_ok_gt_as_result!($a_function, $b_expr) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    }};
