...
///
/// * [`read_to_string_eq`]
/// * [`read_to_string_eq_as_result`]
/// * [`debug_read_to_string_eq`]
///
#[macro_export]
...
mod tests {
    ...
    fn test_read_to_string_eq_as_result_x_success() {
        ...
        let x = assert_fs_read_to_string_eq_as_result!(&a, &b);
        assert_eq!(x, Ok(()));
    }
...
macro_rules! assert_fs_read_to_string_eq {
    ($a_path:expr, $b_path:expr $(,)?) => ({
        match read_to_string_eq_as_result!($a_path, $b_path) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a std::fs::read_to_string() value is equal to another.
///
/// This macro provides the same statements as [`read_to_string_eq`],
