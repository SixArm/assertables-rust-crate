...
macro_rules! assert_fn_ok_ne_as_result {
    ($function:path, $a_input:expr, $b_input:expr $(,)?) => ({
        ...
        if !a_is_ok || !b_is_ok {
            Err(format!(
                concat!(
                    "assertion failed: `assert_fn_err_ne!(pair_function, left_input, right_input)`\n",
                    " pair_function label: `{}`,\n",
                    "    left_input label: `{}`,\n",
                    "    left_input debug: `{:?}`,\n",
                    "   right_input label: `{}`,\n",
            ...
            if a_ok != b_ok {
                ...
                Err(format!(
                    concat!(
                        "assertion failed: `assert_fn_ok_ne!(pair_function, left_input, right_input)`\n",
                        " pair_function label: `{}`,\n",
                        "    left_input label: `{}`,\n",
                        "    left_input debug: `{:?}`,\n",
                        "   right_input label: `{}`,\n",
...
mod test_x_result {
    ...
    fn test_assert_fn_ok_ne_as_result_x_success() {
        let a: i32 = 1;
        let b: i32 = 2;
        let x = assert_fn_ok_ne_as_result!(example_digit_to_string, a, b);
        assert!(x.is_ok());
        assert_eq!(x, Ok(()));
    }
...
macro_rules! assert_fn_ok_ne {
    ($function:path, $a_input:expr, $b_input:expr $(,)?) => ({
        match assert_fn_ok_ne_as_result!($function, $a_input, $b_input) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a function ok() is not equal to another.
///
/// This macro provides the same statements as [`assert_fn_ok_ne`],
