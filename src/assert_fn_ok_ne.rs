...
mod test_x_result {
    ...
    #[test]
    fn test_assert_fn_ok_ne_as_result_x_arity_2_success() {
        let a: i32 = 1;
        let b = String::from("2");
        let x = assert_fn_ok_ne_as_result!(example_digit_to_string, a, b);
        assert_eq!(
    ...
    #[test]
    fn test_assert_fn_ok_ne_as_result_x_arity_2_failure() {
        let a: i32 = 1;
        let b = String::from("1");
        let x = assert_fn_ok_ne_as_result!(example_digit_to_string, a, b);
        assert_eq!(
...
macro_rules! assert_fn_ok_ne {
    ($function:path, $a_input:expr, $b_expr:expr $(,)?) => ({
        match assert_fn_ok_ne_as_result!($function, $a_input, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
