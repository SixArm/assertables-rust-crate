...
//! # Module macros
//!
//! * [`assert_program_args_stdout_gt`](macro.assert_program_args_stdout_gt.html)
//! * [`assert_program_args_stdout_gt_as_result`](macro.assert_program_args_stdout_gt_as_result.html)
//! * [`debug_assert_program_args_stdout_gt`](macro.debug_assert_program_args_stdout_gt.html)
...
mod tests {
    ...
    fn test_assert_program_args_stdout_gt_as_result_x_success() {
        ...
        let b_program = "bin/printf-stdout";
        let b_args = ["%s%s%s%s%s", "h", "a", "l", "l", "o"];
        let x = assert_program_args_stdout_gt_as_result!(&a_program, &a_args, &b_program, &b_args);
        assert_eq!(x.unwrap(), ());
    }
...
macro_rules! assert_program_args_stdout_gt {
    ($a_program:expr, $a_args:expr, $b_program:expr, $b_args:expr $(,)?) => ({
        match assert_program_args_stdout_gt_as_result!($a_program, $a_args, $b_program, $b_args) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
