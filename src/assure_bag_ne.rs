/// Assure a bag is not equal to another.
///
/// * When true, return `Ok(())`.
///
...
/// let x = assure_bag_ne!(&a, &b);
/// assert!(x.is_err());
/// assert_eq!(x.unwrap_err(), "assurance failed: `assure_bag_ne!(left, right)`\n  left: `[1, 1]`,\n right: `[1, 1]`".to_string());
/// # }
/// ```
///
        ...
        match (&$left, &$right) {
            (left_val, right_val) => {
                ...
                if left_bag != right_bag {
                    ...
                    Err(format!("assurance failed: `assure_bag_ne!(left, right)`\n  left: `{:?}`,\n right: `{:?}`", $left, $right))
                }
            }
        }
    });
...
mod tests {
    ...
    fn test_assure_bag_ne_x_arity_2_failure() {
        ...
        assert_eq!(
            x.unwrap_err(),
            "assurance failed: `assure_bag_ne!(left, right)`\n  left: `[1, 1]`,\n right: `[1, 1]`"
        );
    }
