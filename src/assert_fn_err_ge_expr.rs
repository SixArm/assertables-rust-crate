...
mod test_x_result {
    ...
    #[test]
    fn test_assert_fn_err_ge_expr_as_result_x_success_because_gt() {
        let a: i32 = 20;
    ...
    #[test]
    fn test_assert_fn_err_ge_expr_as_result_x_success_because_eq() {
        let a: i32 = 10;
        let b = String::from("10 is out of range");
        let x = assert_fn_err_ge_expr_as_result!(example_digit_to_string, a, b);
        assert!(x.is_ok());
...
macro_rules! assert_fn_err_ge_expr {
    ($a_function:path, $a_input:expr, $b_expr:expr $(,)?) => ({
        match assert_fn_err_ge_expr_as_result!($a_function, $a_input, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
