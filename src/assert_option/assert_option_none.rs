                    ...
                    _ => {
                        Err(format!(
                            concat!(
                                ...
                                "https://docs.rs/assertables/", env!("CARGO_PKG_VERSION"), "/assertables/macro.assert_option_none.html\n",
                                " a label: `{}`,\n",
                                " a debug: `{:?}`",
                            ),
                            stringify!($a),
...
/// # });
/// // assertion failed: `assert_option_none!(a)`
/// // https://docs.rs/assertables/8.17.0/assertables/macro.assert_option_none.html
/// //  a label: `a`,
/// //  a debug: `Some(1)`
...
macro_rules! assert_option_none {
    ($a:expr $(,)?) => {{
        match $crate::assert_option_none_as_result!($a) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    }};
