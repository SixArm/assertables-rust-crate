...
macro_rules! assert_fn_le_expr_as_result {
    ...
    ($a_function:path, $b_expr:expr $(,)?) => ({
        ...
        if a_output <= $b_expr {
            ...
            Err(format!(
                concat!(
                    "assertion failed: `assert_fn_le_expr!(left_function, right_expr)`\n",
                    " left_function label: `{}`,\n",
                    "    right_expr label: `{}`,\n",
                    "    right_expr debug: `{:?}`,\n",
                    "                left: `{:?}`,\n",
...
mod test_assert_x_result {
    ...
    #[test]
    fn test_assert_fn_le_expr_as_result_x_arity_0_x_success_because_lt() {
        let b: i8 = 2;
        let x = assert_fn_le_expr_as_result!(one, b);
        assert!(x.is_ok());
        assert_eq!(x, Ok(()));
    }

    #[test]
    fn test_assert_fn_le_expr_as_result_x_arity_0_x_success_because_eq() {
        let b: i8 = 1;
        let x = assert_fn_le_expr_as_result!(one, b);
        assert!(x.is_ok());
        assert_eq!(x, Ok(()));
    ...
    #[test]
    fn test_assert_fn_le_expr_as_result_x_arity_0_x_failure_because_gt() {
        let b: i8 = 1;
        let x = assert_fn_le_expr_as_result!(two, b);
        assert!(x.is_err());
        assert_eq!(
            x.unwrap_err(),
            concat!(
    ...
    #[test]
    fn test_assert_fn_le_expr_as_result_x_arity_1_x_success_because_lt() {
        let a: i32 = -1;
        let b: i32 = 2;
    ...
    #[test]
    fn test_assert_fn_le_expr_as_result_x_arity_1_x_success_because_eq() {
        let a: i32 = -1;
        let b: i32 = 1;
        let x = assert_fn_le_expr_as_result!(i32::abs, a, b);
        assert!(x.is_ok());
    ...
    #[test]
    fn test_assert_fn_le_expr_as_result_x_arity_1_x_failure_because_gt() {
        let a: i32 = -2;
        let b: i32 = 1;
        let x = assert_fn_le_expr_as_result!(i32::abs, a, b);
        assert!(x.is_err());
...
macro_rules! assert_fn_le_expr {
    ...
    ($a_function:path, $a_input:expr, $b_expr:expr $(,)?) => ({
        match assert_fn_le_expr_as_result!($a_function, $a_input, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
