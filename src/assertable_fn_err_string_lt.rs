...
/// # fn main() {
/// let x = assertable_fn_err_string_lt!(f, 1, 2);
/// //-> Ok(())
/// assert_eq!(x.unwrap(), ());
///
/// let x = assertable_fn_err_string_lt!(f, 2, 1);
/// //-> Err("…")
/// // assertable failed: `assertable_fn_err_string_lt!(fn, left, right)`
/// //    left input: `2`,
/// //   right input: `1`,
/// //   left output: `\"2\"`,
/// //  right output: `\"1\"`
/// assert_eq!(x.unwrap_err(), "assertable failed: `assertable_fn_err_string_lt!(fn, left, right)`\n   left input: `2`,\n  right input: `1`,\n  left output: `\"2\"`,\n right output: `\"1\"`".to_string());
/// # }
/// ```
///
...
macro_rules! assertable_fn_err_string_lt {
    ($function:path, $left:expr, $right:expr $(,)?) => ({
        ...
        if !left.is_err() || !right.is_err() {
            Err(format!("assertable failed: `assertable_fn_err_string_lt!(fn, left, right)`\n   left input: `{:?}`,\n  right input: `{:?}`\n  left output is_err(): `{:?}`,\n right output is_err(): `{:?}`", $left, $right, left.is_err(), right.is_err()))
        } else {
...
mod tests {
    ...
    fn test_assertable_fn_err_string_lt_x_arity_2_lt_success() {
        let a = 1;
        let b = 2;
        let x = assertable_fn_err_string_lt!(f, a, b);
        assert_eq!(
            x.unwrap(),
            ()
        );
    ...
    fn test_assertable_fn_err_string_lt_x_arity_3_lt_success() {
        let a = 1;
        let b = 2;
        let x = assertable_fn_err_string_lt!(f, a, b, "message");
        assert_eq!(
            x.unwrap(),
            ()
        );
