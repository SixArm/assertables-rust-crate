...
//! assert_fs_read_to_string_gt!(&path, &value);
//! # }
//! ```
//!
//! # Module macros
                    ...
                    Ok(a_string) => {
                        ...
                        if a_string > b_string {
                            ...
                            Err(
                                format!(
                                    concat!(
                                        "assertion failed: `assert_fs_read_to_string_gt!(a_path, b_expr)`\n",
                                        "https://docs.rs/assertables/9.0.0/assertables/macro.assert_fs_read_to_string_gt.html\n",
                                        " a_path label: `{}`,\n",
                                        " a_path debug: `{:?}`,\n",
                                        " b_expr label: `{}`,\n",
                                        " b_expr debug: `{:?}`,\n",
...
mod tests {
    ...
    #[test]
    fn test_read_to_string_gt_expr_as_result_x_success_because_gt() {
        let path = DIR.join("bravo.txt");
        let value = String::from("alfa\n");
        let result = assert_fs_read_to_string_gt_as_result!(&path, &value);
        assert_eq!(
            result.unwrap(),
            (String::from("bravo\n"), String::from("alfa\n"))
        );
    ...
    #[test]
    fn test_read_to_string_gt_expr_as_result_x_failure_because_eq() {
    ...
    #[test]
    fn test_read_to_string_gt_expr_as_result_x_failure_because_lt() {
        let path = DIR.join("alfa.txt");
...
macro_rules! assert_fs_read_to_string_gt {
    ($a_path:expr,  $b_expr:expr $(,)?) => {{
        match $crate::assert_fs_read_to_string_gt_as_result!($a_path, $b_expr) {
            Ok(x) => x,
            Err(err) => panic!("{}", err),
        }
    }};
