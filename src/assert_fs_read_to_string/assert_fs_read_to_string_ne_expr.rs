                ...
                if let Err(a_err) = a_result {
                    Err(format!(
                        concat!(
                            ...
                            "https://docs.rs/assertables/", env!("CARGO_PKG_VERSION"), "/assertables/macro.assert_fs_read_to_string_ne_expr.html\n",
                            " a_path label: `{}`,\n",
                            " a_path debug: `{:?}`,\n",
                            " b_expr label: `{}`,\n",
                            " b_expr debug: `{:?}`,\n",
...
/// # });
/// // assertion failed: `assert_fs_read_to_string_ne_expr!(a_path, b_expr)`
/// // https://docs.rs/assertables/8.14.0/assertables/macro.assert_fs_read_to_string_ne_expr.html
/// //  a_path label: `&path`,
...
macro_rules! assert_fs_read_to_string_ne_expr {
    ($a_path:expr, $b_expr:expr $(,)?) => {{
        match $crate::assert_fs_read_to_string_ne_expr_as_result!($a_path, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    }};
