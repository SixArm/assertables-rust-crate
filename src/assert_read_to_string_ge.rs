...
/// * [`assert_read_to_string_ge_as_result`]
/// * [`debug_assert_read_to_string_ge`]
///
#[macro_export]
macro_rules! assert_read_to_string_ge_as_result {
    ($a_reader:expr, $b_reader:expr $(,)?) => ({
        ...
        if a_result.is_err() || b_result.is_err() {
            Err(format!(
                concat!(
                    "assertion failed: `assert_read_to_string_ge!(left_reader, right_reader)`\n",
                    "  left_reader label: `{}`,\n",
                    "  left_reader debug: `{:?}`,\n",
                    " right_reader label: `{}`,\n",
                    " right_reader debug: `{:?}`,\n",
...
mod tests {
    ...
    #[test]
    fn test_assert_read_to_string_ge_as_result_x_success() {
        let mut a = "bravo".as_bytes();
        let mut b = "alpha".as_bytes();
        let x = assert_read_to_string_ge_as_result!(a, b);
        assert_eq!(x, Ok(()));
    }

    #[test]
    fn test_assert_read_to_string_ge_as_result_x_failure() {
...
macro_rules! assert_read_to_string_ge {
    ($a_reader:expr, $b_reader:expr $(,)?) => ({
        match assert_read_to_string_ge_as_result!($a_reader, $b_reader) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a std::io::Read read_to_string() value is greater than or equal to another.
///
/// This macro provides the same statements as [`assert_read_to_string_ge`],
...
///
#[macro_export]
macro_rules! debug_assert_read_to_string_ge {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_read_to_string_ge!($($arg)*);
        }
    };
}
