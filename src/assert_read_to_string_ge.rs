...
mod test_x_panic {
    ...
    fn test_assert_read_to_string_ge_x_arity_2_success() {
        let mut reader = "bravo".as_bytes();
        let value = String::from("alpha");
        let x = assert_read_to_string_ge!(reader, value);
        assert_eq!(x, ());
    }

    #[test]
    #[should_panic (expected = "assertion failed: `assert_read_to_string_ge!(left_reader, right_expr)`\n left reader name: `reader`,\n  right expr name: `value`,\n left reader size: `5`,\n left reader data: `\"alpha\"`,\n       right expr: `\"bravo\"`")]
    fn test_assert_read_to_string_ge_x_arity_2_failure() {
        let mut reader = "alpha".as_bytes();
        let value = String::from("bravo");
        let _x = assert_read_to_string_ge!(reader, value);
    }
    ...
    fn test_assert_read_to_string_ge_x_arity_3_success() {
        let mut reader = "bravo".as_bytes();
        let value = String::from("alpha");
        let x = assert_read_to_string_ge!(reader, value, "message");
        assert_eq!(x, ());
    }
    ...
    fn test_assert_read_to_string_ge_x_arity_3_failure() {
        let mut reader = "alpha".as_bytes();
        let value = String::from("bravo");
        let _x = assert_read_to_string_ge!(reader, value, "message");
    }
