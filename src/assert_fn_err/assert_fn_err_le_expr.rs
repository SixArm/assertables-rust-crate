...
//! # Module macros
//!
//! * [`assert_fn_err_le_expr`](macro@crate::assert_fn_err_le_expr)
//! * [`assert_fn_err_le_expr_as_result`](macro@crate::assert_fn_err_le_expr_as_result)
//! * [`debug_assert_fn_err_le_expr`](macro@crate::debug_assert_fn_err_le_expr)
                ...
                if !a_is_err {
                    Err(format!(
                        concat!(
                            ...
                            "https://docs.rs/assertables/", env!("CARGO_PKG_VERSION"), "/assertables/macro.assert_fn_err_le_expr.html\n",
                            " a_function label: `{}`,\n",
                            "    a_param label: `{}`,\n",
                            "    a_param debug: `{:?}`,\n",
                            "     b_expr label: `{}`,\n",
...
mod tests {
    ...
    mod assert_fn_err_le_expr_as_result {
        ...
        mod arity_1 {
            ...
            fn test_lt() {
                let a: i8 = 1;
                let b: i8 = 2;
                let result = assert_fn_err_le_expr_as_result!(f, a, b);
                assert_eq!(result, Ok(()));
            }
        ...
        mod arity_0 {
            ...
            fn test_lt() {
                let b: i8 = 2;
                let result = assert_fn_err_le_expr_as_result!(f, b);
                assert_eq!(result, Ok(()));
            }
...
/// # });
/// // assertion failed: `assert_fn_err_le_expr!(a_function, a_param, b_expr)`
/// // https://docs.rs/assertables/8.14.0/assertables/macro.assert_fn_err_le_expr.html
/// //  a_function label: `f`,
/// //     a_param label: `a`,
/// //     a_param debug: `20`,
/// //      b_expr label: `b`,
...
macro_rules! assert_fn_err_le_expr {
    ...
    ($a_function:path, $a_param:expr, $b_expr:expr $(,)?) => {{
        match $crate::assert_fn_err_le_expr_as_result!($a_function, $a_param, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    }};
    ...
    ($a_function:path, $b_expr:expr $(,)?) => {{
        match $crate::assert_fn_err_le_expr_as_result!($a_function, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    }};
...
/// (function(param) ⇒ Err(a) ⇒ a) ≤ expr
///
/// This macro provides the same statements as [`assert_fn_err_le_expr`](macro.assert_fn_err_le_expr.html),
...
macro_rules! debug_assert_fn_err_le_expr {
    ($($arg:tt)*) => {
        if $crate::cfg!(debug_assertions) {
            $crate::assert_fn_err_le_expr!($($arg)*);
        }
    };
}
