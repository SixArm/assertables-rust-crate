                ...
                if !a_is_err || !b_is_err {
                    Err(format!(
                        concat!(
                            ...
                            "https://docs.rs/assertables/8.18.0/assertables/macro.assert_fn_err_le.html\n",
                            " a_function label: `{}`,\n",
                            "    a_param label: `{}`,\n",
                            "    a_param debug: `{:?}`,\n",
                            " b_function label: `{}`,\n",
    ...
    mod assert_fn_err_le_as_result {
        ...
        mod arity_1 {
            ...
            fn test_lt() {
                let a: i8 = 1;
                let b: i8 = 2;
                let result = assert_fn_err_le_as_result!(f, a, g, b);
                assert_eq!(result, Ok(()));
            }
        ...
        mod arity_0 {
            ...
            #[test]
            fn test_lt() {
                let result = assert_fn_err_le_as_result!(f, g);
                assert_eq!(result, Ok(()));
            }

            #[test]
            fn test_eq() {
                let result = assert_fn_err_le_as_result!(f, f);
                assert_eq!(result, Ok(()));
            }

            #[test]
            fn test_gt() {
                let result = assert_fn_err_le_as_result!(g, f);
                assert!(result.is_err());
                assert_eq!(
                    result.unwrap_err(),
                    concat!(
