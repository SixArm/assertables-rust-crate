                ...
                if !a_is_err {
                    Err(format!(
                        concat!(
                            ...
                            "https://docs.rs/assertables/", env!("CARGO_PKG_VERSION"), "/assertables/macro.assert_fn_err_ne_expr.html\n",
                            " a_function label: `{}`,\n",
                            "    a_param label: `{}`,\n",
                            "    a_param debug: `{:?}`,\n",
                            "     b_expr label: `{}`,\n",
...
/// # });
/// // assertion failed: `assert_fn_err_ne_expr!(a_function, a_param, b_expr)`
/// // https://docs.rs/assertables/8.17.0/assertables/macro.assert_fn_err_ne_expr.html
/// //  a_function label: `f`,
/// //     a_param label: `a`,
/// //     a_param debug: `10`,
/// //      b_expr label: `b`,
...
macro_rules! assert_fn_err_ne_expr {
    ...
    ($a_function:path, $a_param:expr, $b_expr:expr $(,)?) => {{
        match $crate::assert_fn_err_ne_expr_as_result!($a_function, $a_param, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    }};
    ...
    ($a_function:path, $b_expr:expr $(,)?) => {{
        match $crate::assert_fn_err_ne_expr_as_result!($a_function, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    }};
