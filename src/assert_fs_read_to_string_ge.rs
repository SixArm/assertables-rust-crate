...
///
/// * [`read_to_string_ge`]
/// * [`read_to_string_ge_as_result`]
/// * [`debug_read_to_string_ge`]
///
#[macro_export]
...
macro_rules! assert_fs_read_to_string_ge {
    ($a_path:expr, $b_path:expr $(,)?) => ({
        match read_to_string_ge_as_result!($a_path, $b_path) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a std::fs::read_to_string() value is greater than or equal to another.
///
/// This macro provides the same statements as [`read_to_string_ge`],
