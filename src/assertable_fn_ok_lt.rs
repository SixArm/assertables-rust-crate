...
///
/// # fn main() {
/// let x = assertable_fn_ok_lt!(example_digit_to_string, 1, 2);
/// //-> Ok(())
/// assert_eq!(x.unwrap(), ());
///
/// let x = assertable_fn_ok_lt!(example_digit_to_string, 2, 1);
/// //-> Err("…")
/// // assertable failed: `assertable_fn_ok_lt!(function, left, right)`
/// //      function: `\"example_digit_to_string\"`,
/// //    left input: `2`,
/// //   right input: `1`,
/// //   left output: `\"2\"`,
/// //  right output: `\"1\"`
/// assert_eq!(x.unwrap_err(), "assertable failed: `assertable_fn_ok_lt!(function, left, right)`\n     function: `\"example_digit_to_string\"`,\n   left input: `2`,\n  right input: `1`,\n  left output: `\"2\"`,\n right output: `\"1\"`".to_string());
/// # }
/// ```
///
...
macro_rules! assertable_fn_ok_lt {
    ($function:path, $a:expr, $b:expr $(,)?) => ({
        ...
        if !a_is_ok || !b_is_ok {
            Err(format!("assertable failed: `assertable_fn_ok_lt!(function, left, right)`\n     function: `{:?}`,\n   left input: `{:?}`,\n  right input: `{:?}`\n  left output is ok: `{:?}`,\n right output is ok: `{:?}`", stringify!($function), $a, $b, a_is_ok, b_is_ok))
        } else {
...
mod tests {
    ...
    fn test_assertable_fn_ok_lt_x_arity_2_lt_success() {
        let a = 1;
        let b = 2;
        let x = assertable_fn_ok_lt!(example_digit_to_string, a, b);
        assert_eq!(
            x.unwrap(),
            ()
        );
    ...
    fn test_assertable_fn_ok_lt_x_arity_3_lt_success() {
        let a = 1;
        let b = 2;
        let x = assertable_fn_ok_lt!(example_digit_to_string, a, b, "message");
        assert_eq!(
            x.unwrap(),
            ()
        );
