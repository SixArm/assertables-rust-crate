...
/// //-> Err("…")
/// // assertable failed: `assertable_f_ok_ne!(fn, left, right)`
/// //    left input: `\"1\"`,
/// //   right input: `\"1\"`,
/// //   left output: `1`,
/// //  right output: `1`
/// assert_eq!(x.unwrap_err(), "assertable failed: `assertable_f_ok_ne!(fn, left, right)`\n   left input: `\"1\"`,\n  right input: `\"1\"`,\n  left output: `1`,\n right output: `1`".to_string());
/// # }
/// ```
///
...
macro_rules! assertable_f_ok_ne {
    ($function:path, $left:expr, $right:expr $(,)?) => ({
        ...
        if !left.is_ok() || !right.is_ok() {
            Err(format!("assertable failed: `assertable_f_ok_ne!(fn, left, right)`\n   left input: `{:?}`,\n  right input: `{:?}`\n  left output is_ok(): `{:?}`,\n right output is_ok(): `{:?}`", $left, $right, left.is_ok(), right.is_ok()))
        } else {
...
mod tests {
    ...
    fn test_assertable_f_ok_ne_x_arity_2_success() {
        let a = "1";
        let b = "2";
        let x = assertable_f_ok_ne!(i32::from_str, a, b);
        assert_eq!(
            x.unwrap(),
            ()
        );
    ...
    fn test_assertable_f_ok_ne_x_arity_3_success() {
        let a = "1";
        let b = "2";
        let x = assertable_f_ok_ne!(i32::from_str, a, b, "message");
        assert_eq!(
            x.unwrap(),
            ()
        );
