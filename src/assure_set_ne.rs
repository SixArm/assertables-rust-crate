/// Assure a set is not equal to another.
///
/// * When true, return `Ok(())`.
///
...
/// let x = assure_set_ne!(&a, &b);
/// assert!(x.is_err());
/// assert_eq!(x.unwrap_err(), "assurance failed: `assure_set_ne!(left, right)`\n  left: `[1, 2]`,\n right: `[1, 2]`".to_string());
/// # }
/// ```
///
        ...
        match (&$left, &$right) {
            (left_val, right_val) => {
                ...
                if left_set != right_set {
                    ...
                    Err(format!("assurance failed: `assure_set_ne!(left, right)`\n  left: `{:?}`,\n right: `{:?}`", $left, $right))
                }
            }
        }
    });
...
mod tests {
    ...
    fn test_assure_set_ne_x_arity_2_failure() {
        ...
        assert_eq!(
            x.unwrap_err(),
            "assurance failed: `assure_set_ne!(left, right)`\n  left: `[1, 2]`,\n right: `[1, 2]`"
        );
    }
