...
/// # fn main() {
/// let x = assure_fn_err_string_le!(f, 1, 2);
/// assert!(x.is_ok());
/// # }
/// ```
///
...
/// fn f(i: i32) -> Result<bool, String> { Err(format!("{:?}", i)) }
/// # fn main() {
/// let x = assure_fn_err_string_le!(f, 2, 1);
/// assert!(x.is_err());
/// assert_eq!(x.unwrap_err(), "assurance failed: `assure_fn_err_string_le!(fn, left, right)`\n  left input: `2`,\n right input: `1`,\n  left output: `\"2\"`,\n right output: `\"1\"`".to_string());
/// # }
/// ```
///
...
macro_rules! assure_fn_err_string_le {
    ($function:path, $left:expr, $right:expr $(,)?) => ({
        ...
        if !left.is_err() || !right.is_err() {
            Err(format!("assurance failed: `assure_fn_err_string_le!(fn, left, right)`\n  left input: `{:?}`,\n right input: `{:?}`\n  left output is_err(): `{:?}`,\n right output is_err(): `{:?}`", $left, $right, left.is_err(), right.is_err()))
        } else {
...
mod tests {
    ...
    fn test_assure_fn_err_string_le_x_arity_2_lt_success() {
        let a = 1;
        let b = 2;
        let x = assure_fn_err_string_le!(f, a, b);
        assert!(x.is_ok());
    }
    ...
    #[test]
    fn test_assure_fn_err_string_le_x_arity_2_gt_failure() {
        let a = 2;
        let b = 1;
        let x = assure_fn_err_string_le!(f, a, b);
        assert_eq!(
            x.unwrap_err(),
            "assurance failed: `assure_fn_err_string_le!(fn, left, right)`\n  left input: `2`,\n right input: `1`,\n  left output: `\"2\"`,\n right output: `\"1\"`"
        );
    }
    ...
    fn test_assure_fn_err_string_le_x_arity_3_lt_success() {
        let a = 1;
        let b = 2;
        let x = assure_fn_err_string_le!(f, a, b, "message");
        assert!(x.is_ok());
    }
