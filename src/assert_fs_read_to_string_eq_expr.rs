...
/// # Related
///
/// * [`read_to_string_eq_expr`]
/// * [`read_to_string_eq_expr_as_result`]
/// * [`debug_read_to_string_eq_expr`]
///
#[macro_export]
...
macro_rules! assert_fs_read_to_string_eq_expr {
    ($a_path:expr, $b_expr:expr $(,)?) => ({
        match read_to_string_eq_expr_as_result!($a_path, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
...
/// Assert a std::fs::read_to_string() value is equal to an expression.
///
/// This macro provides the same statements as [`read_to_string_eq_expr`],
