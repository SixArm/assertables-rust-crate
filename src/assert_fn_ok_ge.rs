    ...
    ($a_function:path, $b_function:path) => ({
        ...
        if !a_is_ok || !b_is_ok {
            ...
            if a_ok >= b_ok {
                ...
                Err(format!(
                    concat!(
                        "assertion failed: `assert_fn_ok_ge!(left_function, right_function)`\n",
                        "  left_function label: `{}`,\n",
                        " right_function label: `{}`,\n",
                        "                 left: `{:?}`,\n",
                        "                right: `{:?}`"
    ...
    mod assert_fn_ok_ge_as_result {
        ...
        mod arity_1 {
            ...
            fn test_gt() {
                let a: i8 = 2;
                let b: i8 = 1;
                let x = assert_fn_ok_ge_as_result!(f, a, g, b);
                assert_eq!(x, Ok(()));
            }
        ...
        mod arity_0 {
            ...
            #[test]
            fn test_gt() {
                let x = assert_fn_ok_ge_as_result!(g, f);
                assert_eq!(x, Ok(()));
            }

            #[test]
            fn test_eq() {
                let x = assert_fn_ok_ge_as_result!(f, f);
                assert_eq!(x, Ok(()));
            }

            #[test]
            fn test_lt() {
                let x = assert_fn_ok_ge_as_result!(f, g);
                assert!(x.is_err());
                assert_eq!(
                    x.unwrap_err(),
                    concat!(
...
macro_rules! assert_fn_ok_ge {
    ...
    ($a_function:path, $b_function:path) => ({
        match assert_fn_ok_ge_as_result!($a_function, $b_function) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
