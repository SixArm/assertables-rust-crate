...
mod test_x_result {
    ...
    #[test]
    fn test_assert_fn_err_lt_expr_as_result_x_success_because_lt_expr() {
        let a: i32 = 10;
    ...
    #[test]
    fn test_assert_fn_err_lt_expr_as_result_x_failure_because_eq_expr() {
        let a: i32 = 10;
    ...
    #[test]
    fn test_assert_fn_err_lt_expr_as_result_x_failure_because_gt_expr() {
        let a: i32 = 20;
        let b = String::from("10 is out of range");
        let x = assert_fn_err_lt_expr_as_result!(example_digit_to_string, a, b);
        assert!(x.is_err());
...
macro_rules! assert_fn_err_lt_expr {
    ($function:path, $a_input:expr, $b_expr:expr $(,)?) => ({
        match assert_fn_err_lt_expr_as_result!($function, $a_input, $b_expr) {
            Ok(()) => (),
            Err(err) => panic!("{}", err),
        }
    });
