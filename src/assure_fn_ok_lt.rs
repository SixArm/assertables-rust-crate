...
/// use std::str::FromStr;
/// # fn main() {
/// let x = assure_fn_ok_lt!(i32::from_str, "1", "2");
/// assert!(x.is_ok());
/// # }
/// ```
///
...
/// use std::str::FromStr;
/// # fn main() {
/// let x = assure_fn_ok_lt!(i32::from_str, "2", "1");
/// assert!(x.is_err());
/// assert_eq!(x.unwrap_err(), "assurance failed: `assure_fn_ok_lt!(fn, left, right)`\n  left input: `\"2\"`,\n right input: `\"1\"`,\n  left output: `2`,\n right output: `1`".to_string());
/// # }
/// ```
///
...
macro_rules! assure_fn_ok_lt {
    ($function:path, $left:expr, $right:expr $(,)?) => ({
        ...
        if !left.is_ok() || !right.is_ok() {
            Err(format!("assurance failed: `assure_fn_ok_lt!(fn, left, right)`\n  left input: `{:?}`,\n right input: `{:?}`\n  left output is_ok(): `{:?}`,\n right output is_ok(): `{:?}`", $left, $right, left.is_ok(), right.is_ok()))
        } else {
...
mod tests {
    ...
    fn test_assure_fn_ok_lt_x_arity_2_lt_success() {
        let a = "1";
        let b = "2";
        let x = assure_fn_ok_lt!(i32::from_str, a, b);
        assert!(x.is_ok());
    }
    ...
    fn test_assure_fn_ok_lt_x_arity_2_eq_failure() {
        ...
        assert_eq!(
            x.unwrap_err(),
            "assurance failed: `assure_fn_ok_lt!(fn, left, right)`\n  left input: `\"1\"`,\n right input: `\"1\"`,\n  left output: `1`,\n right output: `1`"
        );
    }
    ...
    fn test_assure_fn_ok_lt_x_arity_3_lt_success() {
        let a = "1";
        let b = "2";
        let x = assure_fn_ok_lt!(i32::from_str, a, b, "message");
        assert!(x.is_ok());
    }
