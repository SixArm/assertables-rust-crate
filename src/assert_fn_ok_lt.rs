...
macro_rules! assert_fn_ok_lt_as_result {
    ($a_function:path, $a_input:expr, $b_function:path, $b_input:expr $(,)?) => ({
        ...
        if !a_is_ok || !b_is_ok {
            Err(format!(
                concat!(
                    "assertion failed: `assert_fn_err_lt!(left_function, left_input, right_function, right_input)`\n",
                    "  left_function label: `{}`,\n",
                    "     left_input label: `{}`,\n",
                    "     left_input debug: `{:?}`,\n",
            ...
            if a_ok < b_ok {
                ...
                Err(format!(
                    concat!(
                        "assertion failed: `assert_fn_ok_lt!(left_function, left_input, right_function, right_input)`\n",
                        "  left_function label: `{}`,\n",
                        "     left_input label: `{}`,\n",
                        "     left_input debug: `{:?}`,\n",
                        " right_function label: `{}`,\n",
...
mod test_x_result {
    ...
    #[test]
    fn test_assert_fn_ok_lt_as_result_x_success_because_lt() {
        let a: i32 = 1;
        let b: i32 = 2;
        let x = assert_fn_ok_lt_as_result!(example_digit_to_string, a, example_digit_to_string, b);
        assert!(x.is_ok());
        assert_eq!(x, Ok(()));
    }

    #[test]
    fn test_assert_fn_ok_lt_as_result_x_failure_because_eq() {
        let a: i32 = 1;
        let b: i32 = 1;
        let x = assert_fn_ok_lt_as_result!(example_digit_to_string, a, example_digit_to_string, b);
        assert!(x.is_err());
    ...
    #[test]
    fn test_assert_fn_ok_lt_as_result_x_failure_because_gt() {
        let a: i32 = 2;
        let b: i32 = 1;
        let x = assert_fn_ok_lt_as_result!(example_digit_to_string, a, example_digit_to_string, b);
        assert!(x.is_err());
...
/// Assert a function ok() is less than another.
///
/// This macro provides the same statements as [`assert_fn_ok_lt`],
